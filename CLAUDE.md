...
| Other runtime fragments | `ck::FFragment_[Feature]_[PurposeNoun]` — `_State`, `_Result`, `_Graph`, `_Pending*`, `_*Inbox`, `_*Registry`, `_Cooldowns`, `_Previous`, `_Debug`, … (`_Current` is RETIRED and fully purged as of 2026-08-08 — a reappearance is a regression; never `_Runtime`/`_Tracker`/`_Live`/`_Status`) | `X_Fragment.h` |
| Retained immutable config | `ck::FFragment_[Feature]_Params` — EARNED by steady-state reads, never granted by default; holds ONLY the post-construction-read residue (`Params ⊆ Spec`). Fields matching the Spec 1:1 alias it: `using FFragment_X_Params = FCk_X_Spec;` — EXCEPT when the Spec is empty, where the alias makes the fragment an empty type and trips the `ck::TTag` static_assert at `CkEcs/.../CkRegistry.h`; those keep a wrapper holding `ParamsType _Params` (CkResolverTarget, CkPredictedVelocity) | `X_Fragment.h` |
| Request-mutable config | `ck::FFragment_[Feature]_Tunables` — config a request or the debugger REPLACES at runtime. Not `_Params`: that name promises immutability, and the promise is what lets a processor take the view `TReadOnly`. Reach for it only when there is no immutable residue to split off (CkCrowdAgent, CkAStar, CkPathNetworkFollower, CkVoiceTalker) | `X_Fragment.h` |
