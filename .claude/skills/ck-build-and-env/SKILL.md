...
| T9 | ANY headless `-run=<Commandlet>` aborts with `Cannot run when angelscript has failed to compile. Requesting exit.` (exit 3), naming `::Params()` / `SpawnParams is not declared` errors — and `_StubRecovery_*.as` reappears | The cold boot REGENERATES the generated spawn-params file from an incomplete type database at startup, gutting it (measured: 434,845 → 24,125 bytes, all gym entity-script entries lost), then fails the AS compile against its own output. It is not a stale file being inherited — a known-good file is destroyed by the boot that follows the GUI boot which produced it | **Open the GUI editor once** — it regenerates correctly and self-deletes the stub file. Do NOT re-run the commandlet (it re-clobbers, deterministically, same byte count every time) and do NOT hand-write the missing declarations into `_StubRecovery_*` (the dispatcher rewrites that file every boot, so the edit never reaches the compile that needs it). Both files are gitignored, so nothing tracked is at risk — but every headless commandlet stays blocked until a human opens the editor. [operator experience, 2026-08: three commandlet attempts, three identical clobbers] |
