# Engine, plugin placement, CkModuleRules, building, adding a module

Reference for `ck-build-and-env`: the source-build engine (§1), submodule plugin placement (§2), the shared Build.cs spine (§3), project-file generation / build / headless boot (§4), and adding a new module (§5).
