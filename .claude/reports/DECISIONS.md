...
## Gameplay-cascade settle barrier (campaign 2026-08-21, executed 2026-08-24)

111. **SM transition cascades converge at a `FGroup_Gameplay_Script` settle barrier** → 16 consumed-marker triggers (the CkStateMachine decision/exit chain + `SmScript_CommitPendingAttach` + the four EntityScript pipeline processors), zero replay-only participants; `SmCondition_Polled`/`SmState_Update` are fenced OUT (marker-less participants replay every barrier pass, and Polled's unconditional parent wake makes a guaranteed 2-pass livelock) → cascade-born transform writes take the ordinary drain/push same frame; the polled/vacuous half of the SM stays next-frame by design; #717 remains the floor for tail-pump-born producers. Two corollaries promoted to doctrine (CkEcs/Claude.md): barrier trigger dirtiness is fragment PRESENCE, so every trigger must REMOVE its marker; and `Request_Exit` on a target already in the destruction pipeline no longer tags (its exit is delivered by the Active-deduped EntityScript EndPlay path) — a dying subtree's pending-exit key otherwise pins the barrier at the pass limit. Campaign: `docs/campaigns/2026-08-21-gameplay-cascade-settle-barrier/` → high (red→green executable spec `Bb_AutoTest_Sm_CascadeWriteReachesComponentSameFrame` + livelock guard spec; frame-interleave probes proved the AutoTest step timer runs in the frame's TAIL region, so specs must ignite via a Tick-mode task).
...
## Test/gym step control (campaign 2026-07-26 — CkTests harness)

107. **HFSM drives GYM stations, NOT autotests** → gym station step sequences became real CkStateMachine graphs (one `UCk_Gym_StepState` per step, `UCk_Gym_Dwell` gating transitions, `Script/Common/CkGym_StationSm.as`); autotests instead got a named-step SEQUENCER inside `UCk_AutoTest_Base`. Rationale for the split: 31 autotests test CkStateMachine itself, so putting the SM underneath every test's harness makes an SM regression fail all ~870 tests at once AND destroys the signal from its own suite — the harness must depend on strictly less than what it tests. Secondary: an SM state is a separate child entity script, so a 4-phase test's handles would live on the test object while its phases lived in 4 sibling UCLASSes reaching back for them. Gyms have neither problem and gain the SM debugger's graph walk → [MAINTAINER-DIRECTED scope choice; alternatives "HFSM everywhere" and "gyms only, defer tests" were both offered].
108. **Autotest waits are CONDITION-based, never hop-counted** → `Add_Step_WaitUntil` / standalone `WaitUntil` poll a named predicate per frame with a self-bounding deadline that fires ahead of the engine `TimeLimit`, so a stuck test reports the pending step and condition instead of an anonymous `TimesUp`. The superseded `WaitOneFrame` is a 0.05s timer, not a frame wait: it yields at least one frame but the frame count scales with frame rate, so chaining N hops encodes a guess about PROCESSOR ORDERING into the test. Measured on the first migration (`CkAutoTest_Poi_ExplicitHide_RemovesFromBothProjectors`): 43 frames/742ms → 25 frames/406ms, `_TimeoutSeconds` 9.0 → 5.0 default. `WaitOneFrame` is retained unchanged so the unmigrated corpus still compiles → high.
109. **Gym dwell must NOT use `UCk_SmCondition_Timer`** → that condition adds a plain CkTimer on the condition entity and never consults the owning SM's run status (`CkSmCondition_Timer.cpp:18-27`), so a paused station's dwell still elapses and the SM jumps the instant it resumes — i.e. the `Ck_Gym*_Auto` pause toggle looks broken. `UCk_Gym_Dwell` is an EventDriven condition that re-arms the full dwell while paused, mirroring the in-tree `UCk_SmTest_Condition_AfterDelay`. Framework-side follow-up candidate: a pause-aware timer condition in CkStateMachine would remove the need for the gym-local one → high.
110. **AS predicate out-params must be written through a local copy** → `auto Res = OutResult; Res.Set(...)`, never `OutResult.Set(...)`. AngelScript treats a by-value struct parameter as read-only and rejects the direct call ("Non-const method call on read-only object reference", Script/CLAUDE.md 9.1); `FCk_SharedBool` wraps a shared cell so the copy writes the same bool. Caught by the toolbox's AS_COMPILE_FAILED guard, which correctly rejected a run reporting 873/873 passed that had actually booted on STALE bytecode → high (the guard is the reason the false green was caught at all).
111. **Spec naming + fragment granularity doctrine (supersedes the §4 two-tier description)** → reflected authoring struct is `FCk_<Feature>_Spec` (was `FCk_Fragment_<Feature>_ParamsData`; maintainer-ruled 2026-08-05, F1); the internal immutable fragment KEEPS the `ck::FFragment_<Feature>_Params` name (F7) but with NARROWED semantics: earned by steady-state reads only, holds the post-construction residue (`Params ⊆ Spec`), never start-values. `_Current` is retired for new code — primary state is the bare noun (`FFragment_Timer`, Transform precedent), everything else purpose-named (`_State`/`_Result`/`_Pending*`/`_Cooldowns`/`_Previous`/`_Debug`). One home per datum: a spec field lives in the Params residue XOR mutable state/tags (the CkTimer stale-direction bug — request handlers reading `Params._CountDirection` while runtime flips lived in `FTag_Timer_Countdown` — is the case study). Full design + audit evidence: `docs/specs/2026-08-05-config-naming-and-fragment-granularity-design.md`; migration: `docs/campaigns/spec-fragment-granularity/`. Counterexamples ratified as CORRECT wholesale-Params features: CkTween (5 fields/tick), CkStateMachine (frame-hot authority config), CkProbe matching half (per-contact-pair) → high [MAINTAINER-DIRECTED].
