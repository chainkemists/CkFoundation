...
9. **Candidate/planner role split** *(added post-green-light — discovered scoping the BB pilot; flagged for CTO's next pass)*. A promoted host is BOTH a candidate action in its parent's search AND the planner for its children, but one `_Resolved` field served both roles: `_WorldStateSource_Override` — the only lever that points a sub-tree at a sub-WS — would cache the host's precondition/effect indices in sub-registry space while the parent's snapshot stays in parent space: silent index aliasing in the parent's A*. Unpinned before this: `WSOverride` only asserts resolution identity, never plans, and both its registries happened to register keys in the same order. The split: **an action's preconditions/effects register and resolve against the SEARCHING planner's resolved WS** (tree parent's, else owning planner's); the entity's own `_Resolved` governs only its goal, its snapshot, and its children's inheritance. `DoInjectGoalSynchronous` residency-classifies goal keys (the planner's WS may be a parented sub-WS). Author-facing consequence: `Set_WorldStateSource_Override` is a **planner-role designation** — on a promoted host it scopes the sub-catalog and goal to the sub-WS while the host's own candidate keys stay parent-resident, which is exactly the BB migration wiring. Pinned by the `PromotedHostRoleSplit` test (diverged registry key orders make any cross-registry leak misalign indices and corrupt the plan).

10. **Dead-parent contract.** When `_Parent` no longer resolves (parent WS entity destroyed before the sub-WS), imported keys degrade to **miss semantics**: `Get_Value` returns `false` (the miss-at-chain-root contract), the plan-snapshot merge writes `false` into imported slots (reads and A* must agree), and forwarded writes are **dropped** with the same Verbose diagnostic class as the registry-full drop (`CkGoap_WorldState_Processor.cpp:77-79`). The stale alias slot is never served as truth. *Deviation from the review's parenthetical recommendation ("writes fall through to local"), for coherence: a write that lands locally while reads return `false` is a write that can never be read back — dropping it with a diagnostic is the honest failure. The hard requirements (defined contract, no stale-slot reads, teardown AutoTest) are met either way.*
...
- **Save/load**: verified 2026-08-13 — CkGoap registers **no** CkSnapshot persistence handlers (every "Snapshot" hit in the module is the override-diff helper or active-chain snapshots), so WS values do not persist and there is no save/load exposure. If persistence lands later: registries and `_ImportedTags` rebuild at action setup, truth lives in the parent, and `ParentLink` is composition (re-established by `DoConstruct` replay), so import-aliasing survives a rebuild+hydrate load.
