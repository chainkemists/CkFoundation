...
| Multi-line wrap comparator (T-pages + C1 in the rect suite, 32/32): 25 line-runs, 1 line-count mismatch, mean advance delta 5.02% after applying text-transform in both rendering and measurement (uppercase outlier 26.2% → resolved) | case-transform cycle (CkPlugins_2.log) | [read] |
| Named residuals, quantified: n1 75.3% with Chromium line wider than the content box = explicit `<br>` collapsed by IR content folding (schema gap: break positions unrecorded); n2/n5 14–19% with ~50px shortfall on ~25 chars = letter-spacing signature — Slate's measure path likely ignores FSlateFontInfo::LetterSpacing for legacy fonts | comparator deltas + arithmetic | [read + inferred: verify letter-spacing handling against 5.7 FontMeasure source] |
| Per-side borders baked (penetration-ratio miter classification over an outer-minus-inner rounded SDF ring; per-side colors typed through the schema, B5 diagnostic retired): P1 0.7412% → **0.1695%** — residual is rounded-edge AA only, matching the arithmetic attribution | border-bake cycle scoreboard, 32/32 | [read] |
