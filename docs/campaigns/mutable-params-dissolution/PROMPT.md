...
### Workstream 2 — the Spec-wrapper fragments (24; added 2026-08-06)

A THIRD fragment shape exists beside the alias and the residue, and it is the weakest of the three:
...
private:
    ParamsType _Params;                                          // the WHOLE Spec, wrapped
public:
    CK_PROPERTY_GET(_Params);
    CK_DEFINE_CONSTRUCTORS(FFragment_Camera_Params, _Params);    // <-- a Spec-taking ctor
};
```

`CK_DEFINE_CONSTRUCTORS(..., _Params)` where `_Params` IS the Spec expands to
`explicit FFragment_X_Params(const FCk_X_Spec&)` — **precisely the constructor the root CLAUDE.md
rule forbids**, generated by the macro rather than hand-written. The sweep that reported "zero
Spec-taking ctors remain" searched only for the hand-written form and was blind to this. Find them:

```powershell
Get-ChildItem -Recurse -Include *_Fragment.h -File |
  Select-String -Pattern 'CK_DEFINE_CONSTRUCTORS\(\s*FFragment_\w+_Params\s*,\s*_Params\s*\)'
```

Retention equals the plain alias (the whole Spec), so the wrapper buys nothing while costing a
double hop at every read (`Get_Params().Get_Field()`) plus the forbidden ctor.

**CkCamera is the flagship case and the recommended start for this workstream:**
`FFragment_Camera_Params` wraps the entire `FCk_Camera_Spec` and is read in exactly ONE place —
`CkCamera_Processor.cpp:310`, `Get_Params().Get_DriveControllerControlRotation()`. A whole
authoring struct retained on every camera entity to serve one flag. Residue it; verify the other
Spec fields are construction-only (consumed by `Add` building the tuner attributes and layers)
before dropping them.

### Workstream 3 — mutation through `Handle.Get<>` (scan A is blind to it)

CkCamera also demonstrates a shape neither scan above catches: a processor mutating a Params
fragment it never declared in its view, by reaching through a handle.

- `CkCamera_Processor.cpp:110-112` — `auto& Params = TypedLayer.Get<FFragment_CameraLayer_Params>();`
  then `Set_Priority` / `Set_CameraTarget`.
- `CkCamera_Utils.cpp:51-52` — same shape, `Set_IsDefault` / `Set_Priority`.

Both the mutable-Params defect AND an undeclared-access problem: the scheduler cannot see a write
the processor never declared. Scan C:

```powershell
Get-ChildItem -Recurse -Include *.cpp -File |
  Select-String -Pattern 'Get<\s*(ck::)?FFragment_\w+_Params\s*>\(\)' -Context 0,3 |
  Where-Object { ($_.Context.PostContext -join ' ') -match '\.Set_' -or $_.Line -match '\.Set_' }
```
...
- **This checkout is SHARED and another session moves it.** On 2026-08-06 a sibling session
  checked `dev` out in this submodule mid-campaign, silently switching the working tree off this
  branch. Symptom: scans return the pre-campaign state (no `FCk_*_Spec` names, 113
  `FCk_Fragment_*_ParamsData` structs) and edited files read as unmodified originals. **Verify
  `git branch --show-current` before trusting ANY scan result**, and re-check it after any long
  gap. Nothing was lost — the branch and commits survive a stray checkout; just switch back.
  Note `git reflog` defaults to few entries and will show the OTHER session's activity, which
  makes it look like your work never happened. It did; use `git branch -a` and `git cat-file -e`.
