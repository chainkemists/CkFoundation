...
## Phase F3 (obj-2 — full Model-A purge) — COMPLETE (clusters 2–6; Opus, 2026-07-13)
...
- **F3 cluster 4 — CkEcs teardown (ATOMIC). GATE GREEN (Dev+Net delta-zero); Shipping proof Succeeded (538s; only pre-existing JPH/Jolt LNK4217 warnings, zero snapshot symbols in link diagnostics). COMMIT CkF `1ddd20da6` + CkTests `0e8dd1e`** (unpushed; CkF first). Deleted WHOLE (git rm): 16 machinery files — `CkEcs/Snapshot/{FragmentRegistry,Archive_Writer,Archive_Reader,TagDriver,TagRegistry,FidelityOracle,Audit}.{h,cpp}` + `CkSnapshot_CkEcsFragments_Registration.cpp` + `CkEcs/Concepts/CkSnapshot_Concepts.h` — and the 2 oracle CkTests (`Test_Snapshot_Audit_UnregisteredRoundTrip.cpp` + `Test_Snapshot_Oracle_ProduceDiffBaseline.cpp`). **CkTag.h**: `CK_DEFINE_ECS_TAG` rewritten registrar-free (dropped `Register_SnapshotableTag<T>()` + the TagRegistry.h include across all ~229 tags); `CK_DEFINE_ECS_TAG_TRANSIENT` kept as a zero-churn alias (no call-site churn). Swept the 4 CkEcs-owned fragments (CkHandle.h ContextOwner+DEBUG_NAME, CkEntityLifetime_Fragment.h Owner/Dependents, CkEntityScript_Fragment.{h,cpp} incl. the `_SnapshotLoadPin` member+fill path, CkNet_Fragment.h) — removed SerializeSnapshot/`using IsSnapshotable`/snapshot includes. **Pruned FSnapshotContext** (`CkSnapshot_Context.{h,cpp}`): deleted `_Saver`/`_Loader`/the 2 entt ctors/`IsSaving()`/the `_Loader` branch in `Snapshot_EnttEntity`/the `snapshot.hpp` include; KEPT the live-v3 surface (SnapshotRegistryType, Snapshot_Handle+re-homing, `_SavedIdMap`, `_LoadRegistryHandle`, map-backed+default ctors — used at CkSnapshot_Subsystem.cpp:540 / CaptureV3.cpp:108). Deleted `Get_ProduceHandlerTypes` (kept the live `Get_SaveHandlerTypes`). Dropped the 2 `CkSnapshot_Audit.h` includes + `CK_SNAPSHOT_ANNOUNCE_EXPECTED` from CkEntityHolder_Fragment.h + CkRecord_Fragment.h (kept CkSnapshot_Policy.h inert — [F3-D2] Option A deferred). **`CK_WITH_FIDELITY_ORACLE` now 0 repo-wide.** Edits via a 7-task Workflow (edit + adversarial-review stages, all 14 agents clean) + CkTag.h/git-rm by hand. **Gate:** Dev build Succeeded (FINAL binary); Ck.Snapshot 29 total → 26 pass / **3 fail = the documented expected reds** (`Ck.Snapshot.Parity.GridPlacements_MPReload` engine-death + `Bb.Snapshot.{FixtureReconstruct,PlayerRestore}` pre-existing casualties) — delta-zero. Net 103/102/1 (the single documented `StateMachine.Net.OwningClientAuth_SubSm_AuthorityGatedTask` flake) — delta-zero. Count 54→29 reflects the Model-A test deletions across clusters 2–4 (cluster 5 re-baselines). 4 residual deleted-symbol mentions remain as COMMENTS only (CkActorSpawnIntent_Fragment.h:29, CkHandle.h:313, CkEntityScript_SpawnRecipe.h:26, CaptureV3.cpp:105) — cluster-6 comment sweep.

- **F3 cluster 5 — VALIDATION.md/PHASE_5.md amend + re-baseline. DONE (Opus, 2026-07-13; Fable-adjudicated agent `a7ba892eac`).** VALIDATION §1: dropped 8 dead test-names from the "green BY NAME" list (Oracle.StructuralBaseline/ProduceDiffBaseline — deleted with Model A; the 5 Rebuild.* — never written; Physics.Net.Velocity_ApplyAfterLateSetup — never written, subsumed per [P2-D3]); added the real surviving surface (Meta.RepDataRestoreCoverage, M2a/M2b/M2b2a/M2b2b family, Parity.{Acceleration,AnimPlan,Attributes,InventoryDataOnly,InventorySpatial,RenderTarget,StateMachine,StateMachineNoHistory,TagSet,TeamPlayer}_MPReload — verified present in CkTests this session); added an explicit **allowlisted-reds** block (GridPlacements engine-death + 2 Bb.Snapshot casualties BY NAME) + an audit-trail line; repointed "failing set == Phase-0 baseline" → the re-baselined table. Reworded the Shipping bullet ("deleted, not gated" + FINALIZE exit-greps). **§2 "Oracle acceptance" REPLACED (not deleted) with "Fidelity acceptance (v3 — post-oracle)"** — the oracle's gap-detection job has residue no surviving test covers, so §2 now names the fidelity-gap register as an Adam-decision surface. §4 audit-warning reword; §5 gate-table clause. **PHASE_5.md: SUPERSEDED banner** (whole doc — its gate-not-delete premise is void; FINALIZE.md is authoritative). No code, no gate needed (docs only).
- **[F3-D1b — ADAM coverage loss, NEW]: the "cluster 0 (port-before-delete)" migrate set was NEVER authored, yet F3 cluster 2 deleted its Model-A source tests.** FINALIZE.md (§ "cluster 0", lines ~317/370/388) required porting BEFORE deleting: `Ck.Snapshot.V3.HandleWalk.*` (handle-remap coverage — incl. the tombstone-incident `DynamicFragment_TypedHandleRemap`), `V3.ProduceSensitivity` (replaces the deleted `Oracle.ProduceDiffBaseline`), and `Parity.AttributeModifier_MPReload` (migrates the deleted `FloatAttribute.Gate` — **FINALIZE: "the ONLY modifier-across-save/load coverage — migrate, don't delete"**). None were authored; all their Model-A sources are deleted (cluster 2). FINALIZE's executor note permitted purge-ahead-of-port ONLY with the gap recorded — [F3-D1] recorded only the Grid/Timer/MP trio, so this records the rest. **Not Adam-approved.** Adam: accept the loss, or schedule the 3-suite port (all build against the CURRENT v3 harness — `Test_Snapshot_V3_Capture.cpp` + the Parity spec pattern). Chose RECORD over author-now: net-new test authoring (esp. the AttributeModifier v3 harness) is beyond cluster-5 doc scope, risks introducing reds unattended, and matches the [F3-D1] precedent + the mission's don't-block directive.
- **F3 cluster 6 — comment sweep + FULL exit greps. DONE+COMMITTED CkF `0a5fa240d` + CkTests `0f72339`** (unpushed; CkF first). Swept 7 isolated stale comments referencing hard-deleted symbols (via a 7-task Workflow, edit+adversarial-review; the review stage caught+fixed 3 accuracy defects — a save/load verb misuse in CaptureV3:92, a self-contradictory FFragment_SaveKey clause in CkActorSpawnIntent:29, an inaccurate carrier attribution in CkEntityScript_SpawnRecipe:26): CkMontagePlayer_Utils.{cpp,h} (deleted Params SerializeSnapshot), CkStateMachine_Processor.cpp:1115 (deleted FFragment_Sm_Current::SerializeSnapshot), CkSnapshot_CaptureV3.cpp:92,105 (deleted CkDynamic SerializeSnapshot + FSnapshotArchive_Writer), CkActorSpawnIntent_Fragment.h:29 (FSnapshotArchive_Writer), CkEntityScript_SpawnRecipe.h:26 (_SnapshotLoadPin), CkReplicatedFragmentContainer.h:4,33 (SeedContainer), CkTests GridPlacements/RepDataRestoreCoverage (deleted test-name refs). By hand: retired the 2 framework `CLAUDE.md` macro rows (root macro-table row for `CK_REGISTER_SNAPSHOTABLE` → tombstone; Source/CLAUDE.md decision-table row → v3 Produce/Apply). **Exit-grep END STATE (both plugins, `rg --no-ignore`): 0 references to hard-deleted symbols** (`FSnapshotArchive_`, `_SnapshotLoadPin`, `CK_WITH_FIDELITY_ORACLE`, `FidelityOracle`, `Get_ProduceHandlerTypes`, `Register_SnapshotableTag`, `FSnap_`, `Run_Capture`, TagRegistry/TagDriver) **outside the documented carve-outs below.**
  - **CARVE-OUT scope (executor decision, [F3-D2] extension):** the `CK_REGISTER_SNAPSHOTABLE|SerializeSnapshot|TSnapshotMarker|IsSnapshotable|FSnapshotPolicy` greps still hit the **[F3-D2] Option-A-deferred T_Policy snapshot surface** — `CkEntityHolder_Fragment.h`, `CkEntityHolder_Utils.h`, `CkRecord_Fragment.h`, `CkRecord_Utils.h`, `CkSnapshot_Policy.h`. I read [F3-D2]'s "keep Policy.h + holder/record base inert until Option A" as covering the WHOLE coherent unit: the live `SerializeSnapshot` methods (still compile — call the KEPT `FSnapshotContext::Snapshot_Handle` — but are never invoked now the registry is gone), `TSnapshotMarker`/`IsSnapshotable`/`FSnapshotPolicy`, AND the `CK_REGISTER_SNAPSHOTABLE` guidance strings inside their `static_assert` macros. Surgically de-naming just the macro would leave incoherent half-doctrine ("use ROUNDTRIP" with no "how to register") and risks breaking the `\`-continued macros — Option A retires the unit atomically. Other allowed residuals: the 2 intentional dated **tombstones** (root + Source `CLAUDE.md`, naming the removed macro for discoverability — RestoreMarker.h precedent); `CkSnapshot_Context.h:25` (accurate KEEP-file comment on the retained FSnapshotContext); `CkSnapshot_RestoreMarker.h`/`FTag_Snapshot_JustRestored` ([P5-D3] cross-repo BB); `docs/campaigns`.
  - **No gate (compile+test-neutral):** all CkF edits are `//` comments, comment-on-code/include-line (code bytes identical), or markdown doc rows; the CkTests edits are 1 `//` comment + 1 coverage-note map-VALUE string in `Meta.RepDataRestoreCoverage` (whose contract asserts KEY presence + reason-non-empty, NOT reason content — verified `Get_Deferred()` at MetaTest.cpp:54-68). A comment/doc/annotation change cannot alter the binary or a verdict → the F3-cluster-4 Dev+Net+Shipping gate remains authoritative. Minor: `CkReplicatedFragmentContainer.h:4` `CkEnums.h` include may now be dead post-SeedContainer-removal (left in place — harmless; removal is a separate compile-gated cleanup).
...
## Phase F4 (obj-5 — hygiene: comments + ck::algo) — DONE+COMMITTED (Opus, 2026-07-13)

- **F4 — obj-5 hygiene. DONE+COMMITTED CkF `1773fc027`** (unpushed). ONE commit (FINALIZE's "separate comments-only + ck::algo commits" was impractical — both touch Subsystem.cpp; the non-comment delta is self-evidently just the 2 ck::algo conversions + 1 sanctioned log-string strip, so `git show --stat` still makes the split legible). **Gate:** build Succeeded; Ck.Snapshot 29→26/**3** delta-zero (the 3 documented reds: GridPlacements engine-death + 2 Bb.Snapshot); Net 103/102/1 delta-zero (the documented SM.Net flake).
  - **Comment sweep (15 densest touched files, via a 15-task Workflow, 30 agents, all reviews clean):** stripped campaign scaffolding from COMMENTS — plan/phase/cluster/campaign words, decision tags [P*]/[INV*]/[N1*]/[B1]/[F3-*]/[SM-A], author names, "Model A/B", "spec §", "v2/v3"-as-shorthand — keeping the engineering WHY ("keep the invariant, delete the history"). Verified preserved in the rebuild core (Subsystem.cpp): the ATOMIC settle-pump gate-open race rationale, ctx-owner-first provenance, subtractive-reconciliation PARKS-under-gate, freeze-against-half-rebuilt-world. Fixed the 2 stale-FALSE "DORMANT in Phase 1" notes on the now-live hydration queue (verified live: Subsystem.cpp enqueues → FProcessor_Hydration_Dispatch drains). Reworded CkSnapshot_RestoreMarker.h campaign-free (kept the cross-repo-BB retention reason). **One sanctioned non-comment change** (FINALIZE-allowed): CkStateMachine_Processor.cpp `ck::sm::Log` string — stripped "(v1 scope cut, [P4A-F1])"; 3 `{}` args preserved → behavior-neutral. False positives correctly left: v3 on-disk-format vocab (FCk_Snapshot_HeaderV3 etc.), [INVALID REGISTRY] strings, AnimPlan domain vocab, the intentional tombstones. (Note: the Subsystem sweep agent defensively REVERTED the ck::algo NoneOf as a "scope violation" of its comments-only task — I re-applied it by hand.)
  - **ck::algo pass — 2 clean conversions (each +1 `CkCore/Algorithms/CkAlgorithms.h` include):** `DoIs_TeardownComplete` for-loop → `ck::algo::NoneOf` (Subsystem.cpp — validated by the passing Snapshot load tests), `DoGet_HasUndrainedReplicatedFragments_Recursive` for-loop → `ck::algo::AnyOf` (CkEntityReplicationDriver_Utils.cpp — validated by the passing Net tests; short-circuit semantics equivalent).
  - **[F4-D1 — deferred ck::algo remainder, executor decision]:** FINALIZE obj-5's 6-conversion list PREDATES F1's rebuild+hydrate rewrite, which reshaped the Subsystem candidates (:317-318 Transform-into-TSet, :383-387 ForEach-destroy, :597-605 FindIf) into complex critical-path control flow / mutations — no longer clean ck::algo targets, and the Transform-into-TSet overload was flagged uncertain even in-plan. Refactoring the load-bearing rebuild core for pure style at session-end fails the risk/value test (house rule: don't refactor beyond need; correctness > style). Landed the 2 unambiguous read-only-predicate conversions; deferred the rest. Adam: run a focused ck::algo pass if desired.
  - **[F4-D2 — comment long-tail deferred]:** ~25 touched files carry ≤4 scaffolding hits each — mostly false positives (my locating grep over-matched legit "v3 rebuild+hydrate" format vocab). The 15 dense files (the real noise, incl. Subsystem.cpp's ~40) were swept; the low-density tail is an optional obj-5 finish, not swept.

**F1–F4 COMPLETE + COMMITTED + GREEN (unpushed).** Class-4 change → Adam review MANDATORY before push. Branch `feature/save-load-improvements`.

## Red/flake fixes (post-F4, Adam-requested 2026-07-13)

Adam asked to fix the remaining red + flaky tests. Investigated all four via 3 read-only agents + a Fable architecture ruling (all code-verified). Result: **2 of the 4 fixed; Snapshot 26/3 → 27/2, Net 103/102/1 → 103/103/0.**

- **GridPlacements_MPReload — FIXED. CkF `ecd6f4019`** (+ spec docstring CkTests `8ebc9c1`). **The "nested-FEcsWorld ENGINE-DEATH" framing was STALE** — the current build does not crash (the campaign's teardown-drain mitigated it); the test failed as a plain 2-assert. Real cause: the restored `FCk_RepData_2dGridPlacements` payload's Apply handler only stamped the **ClientOnly** `FFragment_2dGridOccupancy_SyncReplication`, so on the authority host nothing rebuilt the placement record → server cell never Occupied. **Fix = an authority-side load-hydration branch on the 2dGridOccupancy Apply handler** (`Ck2dGridOccupancy_Fragment.cpp`), gated on `FCk_HydrationApplyScope::Get_IsActive()`, that re-drives `Request_AddPlacement` per restored entry — the exact CkInventory **[INV-A]/[F1-D6]** pattern. Fable deviation (verified): do NOT gate on occupant validity (a restored occupant is deterministically `entt::null` — unlabeled ConstructSpawned child → save-transient; `Request_AddPlacement` tolerates it). Construct RE-RUNS on the bridged respawn (composes the cell registry), so no recompose wiring needed. **BOTH server AND client asserts green** — the client converged via the free `Request_AddPlacement`→MayRequireReplication re-arm→AuthorityOnly-push→ClientOnly-sync pipeline (Fable's uncertain client-leg claim resolved green). This overturns [F3-D1]'s "Grid save/load has NO passing coverage" and the allowlisted-red status.
- **OwningClientAuth_SubSm_AuthorityGatedTask (the flake) — FIXED. CkTests `3fdd6de`.** NOT an SM-net race (the feature works — a prior run passed with speed=150 on all 3 worlds). It's the ONLY 3-world net test, hitting a harness ordering bug: `Get_RemoteClientPlayerController(server, idx)` returned the idx-th PC in the server's *iterator* order while `Get_ClientWorld(idx)` sorts by PIEInstance — uncorrelated for >1 remote client. **Fix = correlate the server-side remote PC to `Get_ClientWorld(idx)` by PlayerState identity** (UniqueId, PlayerId fallback for NULL-subsystem PIE), with a single-remote-client fast path that is byte-identical for every 2-world test (verified: all other net tests are 2-world). Net 103/103/0; confirmed deterministic (the fix removes the ordering non-determinism, not a probabilistic patch).
- **Bb.Snapshot.FixtureReconstruct + Bb.Snapshot.PlayerRestore — NOT fixed (correctly deferred).** Pre-existing BusterBlock-PROJECT casualties (`Source/BusterBlock/Tests/SaveLoad/`), NOT campaign regressions (the Model-A purge provably can't affect the v3 runtime path; they were red identically before + after). Root cause: the v3 rebuild finds "0 live entries" to rendezvous against because the post-travel world has no BB driver/GameMode to respawn the probes/player — the still-unbuilt **BB-driver-world / OracleParity** prerequisite. Fix requires CkFoundation v3-rebuild capability for a full BB world after travel **plus a BusterBlock-project GameMode change** — Adam/prereq-blocked, out of scope for a plugin change. Remain as the 2 allowlisted reds.
- **Deferred follow-ups (Adam):** (a) `Request_RecomposeFromSnapshot` (`Ck2dGridSystem_Utils.{h,cpp}`) is now caller-less dead code (Model-A leftover; the Apply branch replaces it) — safe to delete (plain C++, not a UFUNCTION), left only to avoid a gate cycle for pure hygiene. (b) Occupant-handle FIDELITY: restored placements carry invalid occupant refs (no death-watch, `Get_OccupantAt` invalid) — the real fix is persisting the occupant via a GameplayLabel (→ ConstructSpawned → handle remaps), same provenance family as the open [INV-A] item-restore question.
