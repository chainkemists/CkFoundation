...
Every participant must be safe to pump (`SkipPump` is rejected), must precede the barrier in the main graph, and shares the scheduler's normal per-frame pump-pass budget. A barrier runs in the load kernel only when every participant declares `RunsDuringLoad`. A trigger must declare a dirty marker, and that marker must be a consumed queue or other monotonically convergent work — never sticky state such as `FTag_Transform_Updated`. A processor whose view needs a marker that a participant consumes must itself participate, or the replayed participant strips the marker before it runs (the EntityScript Replicate strand, "Replication-driver failure modes" item 3).
...
- **Per-processor wall-clock timing is DEMAND-DRIVEN (since 2026-08-20).** It costs two `FPlatformTime::Seconds()` (QueryPerformanceCounter) calls per processor per frame and is REDUNDANT with the stat system, so it now runs only while a consumer is reading the frame history: `FProcessorScheduler::Get_DebugFrameHistory()` stamps a read-frame and `Get_IsDebugTimingWanted()` keeps collection alive for `DebugTimingKeepAliveFrames` afterwards (sized above the Scheduler Debugger's slowest refresh cadence). **`ck.Scheduler.DebugTiming` now defaults to OFF and FORCES collection on** — use it to capture timings for a window you will inspect later. Entity counts, pump counts, dirty flags and empty-view skips are recorded unconditionally (they are plain stores, and tests read them for past frames); only the elapsed-ms numbers are gated.
...
- **No restore relink is needed.** `_AssociatedEntity` is Transient and set only at spawn, but the v3 loader re-Constructs a FRESH script UObject through the ordinary spawn path, so the back-pointer is assigned at spawn exactly as in a fresh world. (A `Relink_AssociatedEntities_AfterRestore` utility existed for the Model-A byte-image restore, which reused live script objects; it never had a caller under v3 and was deleted.)
...
- **`FTag_Hydration_WasHydratedThisLoad`** is stamped by the load (`UCk_Snapshot_Subsystem::DoHydrate_Enqueue`) on every restored (saved-id-mapped) entity before the load gate opens, beside the hydration quarantine; its one sanctioned use is the restored-vs-replaced discriminator (adopt what the loader rebuilt, or spawn a second copy). Transient, so never captured, but it survives for the entity's whole lifetime — it records that a load touched this entity, NOT that anything is pending, so a consumer reading it to decide when a restored value is readable has to poll from a processor that ticks in the right window and carry its own once-per-feature dedup. `Promise_OnHydrated` is that question asked properly. Its reflected form is `UCk_Utils_Snapshot_UE::Get_WasHydratedThisLoad`; the old alias and its "just restored" accessor are deleted.
- **`ck::FTag_Hydration_Quarantine`** (`CkEcs/Tag/CkTag_HydrationQuarantine.h`) holds a restored entity out of sight while the load still owns its Durable state. Stamped on the whole mapped set at payload ENQUEUE and released on the whole set at once — never per entity, because the exclusion is a view filter rather than a memory barrier, so a released entity reading a still-quarantined sibling's fragment directly is exactly what a per-entity release would allow. It has two bounded escapes (the hydrate frame cap, and unconditionally at load finish), each naming what it forced in `FCk_Snapshot_LoadReport::_QuarantineForced`. **Entering destruction leaves the quarantine AND abandons whatever the entity still had queued** (`Request_DestroyEntity`): the tag is removed — which is why the destruction pipeline needs no exemptions and why an entity destroyed mid-load cannot keep it forever — and any outstanding `FFragment_PendingHydration` entries are counted into `ck::FCtx_HydrationOutcomes::_DestroyedWithEntries` and dropped. Counted because that is the last moment they are knowable; dropped so each is counted once, since the dispatcher's view carries `CK_IGNORE_PENDING_KILL` and would otherwise apply (or the load report would sweep) entries already written off. Its `FCtx_HydrationQuarantine` companion is the O(1) "is a load holding anything right now" answer, so the hot paths cost a context read rather than a pool lookup.
...
3. **Replicate stranded by a settle pass (fixed 2026-09-25).** `FProcessor_EntityScript_Replicate` views its request plus `FTag_EntityScript_FinishConstruction`, and `FProcessor_EntityScript_FinishConstruction` (`RunAfter` Replicate) removes that tag. When the `FGroup_Gameplay_Script` local settle barrier (`a8e7f5025`, 2026-08-24) replayed Spawn → ContinueConstruction → FinishConstruction without Replicate, every `Replicates` script spawned from `DoBeginPlay`, from a nested `DoConstruct`, or from a state-machine callback was built on the server and never replicated — no ensure, no log. Replicate is now a non-trigger settle participant, and FinishConstruction retires any request that survives it, ensuring on a listen/dedicated server because that entity can never reach a client. The Warning-level park sites in item 1 are silent in this failure; do not confuse the two. Pinned by `Ck.EntityScript.Net.AS_EntityScript_ChildSpawnedInBeginPlayReplicates`. General rule: a processor whose view depends on a marker a settle participant consumes must itself be a participant.
