...
- **`FTag_Hydration_WasHydratedThisLoad`** is stamped by the load (`UCk_Snapshot_Subsystem::DoHydrate_Enqueue`) on every restored (saved-id-mapped) entity before the load gate opens, beside the hydration quarantine; game-side rebind processors key off it. Transient, so never captured, but it survives for the entity's whole lifetime — it records that a load touched this entity, NOT that anything is pending, so a consumer reading it to decide when a restored value is readable has to poll from a processor that ticks in the right window and carry its own once-per-feature dedup. `Promise_OnHydrated` is that question asked properly. `FTag_Snapshot_JustRestored` is a deprecated alias of the same type, kept while the game's readers are swept; `UCk_Utils_Snapshot_UE::Get_WasJustRestored` is its reflected form and dies with it.
