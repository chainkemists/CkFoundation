...
|---|---|---|---|
| `DissolveAdd` | `/CkUsf/Looks/DissolveAdd.ush` | `Script/CkUsf/CkUsf_SlashLooks_Assets.as` (the family helper `Usf_DissolveAddParams` + the NS_BasicAttack/projectile looks), `CkUsf_HitLooks_Assets.as` (the hit/impact looks), `CkUsf_CastLooks_Assets.as` (the cast/spawn looks) and `CkUsf_ExplosionLooks_Assets.as` (the scorch decal + the two bomb-shell paints) | 26 looks. Additive-reading unlit translucency: a shape mask tinted by Particle Color, eroded by a panning noise whose threshold rides the Niagara dynamic parameter |
