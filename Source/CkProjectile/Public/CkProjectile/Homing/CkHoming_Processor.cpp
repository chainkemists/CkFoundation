...
namespace ck
{
        ...
        -> void
    {
        ...
        [&](FFragment_Homing_Requests& InRequests)
        {
            ...
            [&](const auto& InRequest)
            {
                ...
                if (DoHandleRequest(InHandle, InHoming, InRequest))
        ...
        -> bool
    {
        ...
        InHoming._TargetEntity = Target;
        InHoming._TargetMode = ECk_Homing_TargetMode::Entity;
        InHoming._LocalOffset = FVector::ZeroVector;
        ...
        if (InRequest.Get_TargetPoint() == ECk_Homing_TargetPoint::WorldSpacePointOnTarget)
        {
            ...
            InHoming._LocalOffset = TargetTransform.InverseTransformPosition(InRequest.Get_WorldSpacePoint());
        }

        InHoming._HasPreviousTargetLocation = false;
        InHoming._PreviouslyClosing = false;
        InHoming._MissNotified = false;
    ...
    auto
        ...
        -> bool
    {
        InHoming._TargetEntity = FCk_Handle{};
        InHoming._TargetMode = ECk_Homing_TargetMode::WorldPoint;
        InHoming._WorldPoint = InRequest.Get_TargetLocation();
        InHoming._LocalOffset = FVector::ZeroVector;
    ...
    auto
        ...
        -> bool
    {
        if (InHoming._TargetMode != ECk_Homing_TargetMode::None)
        {
            homing_detail::RestoreBaseAcceleration(InHandle, InHoming._BaseAcceleration);
        }

        InHoming._TargetEntity = FCk_Handle{};
        InHoming._TargetMode = ECk_Homing_TargetMode::None;
        InHoming._HasPreviousTargetLocation = false;
    ...
    auto
        ...
        -> bool
    {
        InHoming._ImpactTiming = InRequest.Get_ImpactTiming();
        InHoming._DesiredTimeToImpactRemaining = InRequest.Get_DesiredTimeToImpact();
        ...
        -> void
    {
        ...
        switch (const auto TargetMode = InHoming.Get_TargetMode())
        {
            case ECk_Homing_TargetMode::Entity:
            {
                const auto& Target = InHoming.Get_TargetEntity();
                ...
                if (ck::Is_NOT_Valid(Target) || NOT UCk_Utils_Transform_UE::Has(Target))
                {
                    ...
                    DoDeactivate(InHandle, InHoming);
                ...
                const auto TargetTransformHandle = UCk_Utils_Transform_UE::CastChecked(Target);
                const auto TargetTransform = UCk_Utils_Transform_UE::Get_EntityCurrentTransform(TargetTransformHandle);
                TargetLocation = TargetTransform.TransformPosition(InHoming._LocalOffset);
                ...
                else if (InHoming._HasPreviousTargetLocation)
                {
                    TargetVelocity = (TargetLocation - InHoming._PreviousTargetLocation) / InDeltaT.Get_Seconds();
                }

                InHoming._PreviousTargetLocation = TargetLocation;
                InHoming._HasPreviousTargetLocation = true;

                break;
            }
            case ECk_Homing_TargetMode::WorldPoint:
            {
                TargetLocation = InHoming._WorldPoint;
                break;
            }
        ...
        InHoming._LastGuidanceState = GuidanceState;

        auto Settings = InParams.Get_GuidanceSettings();
        Settings.Set_ImpactTiming(InHoming._ImpactTiming);
        Settings.Set_DesiredTimeToImpact(InHoming._DesiredTimeToImpactRemaining);

        if (InHoming._ImpactTiming == ECk_Homing_ImpactTiming::ArriveAtDesiredTime)
        {
            InHoming._DesiredTimeToImpactRemaining -= InDeltaT;
        }

        const auto HomingAcceleration = pronav::Compute_HomingAcceleration(
            GuidanceState, Kinematics, Settings, InHoming._BaseAcceleration, InDeltaT);

        auto AccelerationHandle = UCk_Utils_Acceleration_UE::CastChecked(InHandle);
        UCk_Utils_Acceleration_UE::Request_OverrideAcceleration(
            AccelerationHandle, InHoming._BaseAcceleration + HomingAcceleration, {});

        DoCheckForMissedTarget(InHandle, InParams, InHoming, GuidanceState);
    }
        ...
        -> void
    {
        ...
        InHoming._PreviouslyClosing = InState.Get_ClosingSpeed() > 0.0;
    }
}
