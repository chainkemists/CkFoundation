...
auto
    ...
    -> UScriptStruct*
{
    for (auto* Class = InEntityScriptClass;
         ck::IsValid(Class, ck::IsValid_Policy_NullptrOnly{}) && Class != UObject::StaticClass();
         Class = Class->GetSuperClass())
    {
        if (NOT Class->HasMetaData(TEXT("CkSpawnParams")))
        { continue; }

        auto StructName = Class->GetMetaData(TEXT("CkSpawnParams"));
    ...
    -> void
{
    ...
    const auto& CreatePinFromStructProperty = [this](const FProperty* InProperty, const uint8* InStructData)
    {
        auto* Pin = CreatePin(EGPD_Input, NAME_None, InProperty->GetFName());
        _PinsGeneratedForEntityScript.Add(Pin);
        ...
        {
            auto DefaultValueAsString = FString{};
            const auto& DefaultValueSet = FBlueprintEditorUtils::PropertyValueToString(InProperty, InStructData, DefaultValueAsString, nullptr);
            check(DefaultValueSet);
            ...
            K2Schema->SetPinAutogeneratedDefaultValue(Pin, DefaultValueAsString);
                 ...
                 ck::IsValid(StructProperty))
        {
            if (const auto* StructData = InProperty->ContainerPtrToValuePtr<uint8>(InStructData))
            {
                auto DefaultStructValue = FString{};
                StructProperty->Struct->ExportText(
                    DefaultStructValue,
                    StructData,
                    StructData,
    ...
    if (auto* NativeStruct = DoTryGet_NativeSpawnParamsStruct(InEntityScriptClass);
        ck::IsValid(NativeStruct, ck::IsValid_Policy_NullptrOnly{}))
    {
        auto StructDefaultData = TArray<uint8>{};
        StructDefaultData.SetNumZeroed(NativeStruct->GetStructureSize());
        NativeStruct->InitializeStruct(StructDefaultData.GetData());

        for (TFieldIterator<FProperty> It(NativeStruct); It; ++It)
        {
            CreatePinFromStructProperty(*It, StructDefaultData.GetData());
        }

        NativeStruct->DestroyStruct(StructDefaultData.GetData());
        return;
    }
    ...
    -> void
{
        ...
        ck::IsValid(ClassToSpawn))
    {
        ...
        {
            if (auto* NativeStruct = DoTryGet_NativeSpawnParamsStruct(ClassToSpawn);
                ck::IsValid(NativeStruct, ck::IsValid_Policy_NullptrOnly{}))
            {
                _CachedSpawnParamsStruct = NativeStruct;
            }
            else if (auto* EntityScriptSubsystem = GEngine->GetEngineSubsystem<UCk_EntityScript_Subsystem_UE>();
    ...
    -> void
{
    ...
    {
        if (auto* NativeStruct = DoTryGet_NativeSpawnParamsStruct(EntityScriptClass);
            ck::IsValid(NativeStruct, ck::IsValid_Policy_NullptrOnly{}) && NativeStruct != _CachedSpawnParamsStruct.Get())
        {
            _CachedSpawnParamsStruct = NativeStruct;
    ...
    -> FText
{
        ...
        ck::IsValid(EntityScriptClass))
    {
        ...
        const auto& ReplicationText = [&]() -> const TCHAR*
        {
    ...
    const auto& TryCopyValueOrLinkPin = [&](UEdGraphPin* InPinToCopyOrLinkFrom)
    {
        auto* FoundMakeStructPin = MakeSpawnParamsStruct_Node->FindPinByPredicate([&](const UEdGraphPin* InPin)
        {
            return InPin->PinFriendlyName.ToString() == InPinToCopyOrLinkFrom->PinName.ToString()
                || InPin->PinName == InPinToCopyOrLinkFrom->PinName;
        });
