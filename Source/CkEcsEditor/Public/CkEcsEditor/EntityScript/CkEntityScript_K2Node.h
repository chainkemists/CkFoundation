...
UCLASS(MinimalAPI)
class UCk_K2Node_GetScriptEntity : public UCk_K2Node_UFunction_Base
{
    GENERATED_BODY()
...
public:
    ...
    auto GetMenuCategory() const -> FText override;
    auto IsCompatibleWithGraph(UEdGraph const* InGraph) const -> bool override;
    auto ShouldDrawCompact() const -> bool override;
    auto GetCompactNodeTitle() const -> FText override;
    auto ValidateNodeDuringCompilation(class FCompilerResultsLog& MessageLog) const -> void override;
    // End of K2Node implementation
