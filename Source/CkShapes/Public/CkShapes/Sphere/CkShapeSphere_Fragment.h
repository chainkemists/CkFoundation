#pragma once

#include "CkShapeSphere_Fragment_Data.h"

#include "CkEcs/Fragments/ReplicatedObjects/CkReplicatedObjects_Fragment_Params.h"
#include "CkEcs/Signal/CkSignal_Macros.h"

// --------------------------------------------------------------------------------------------------------------------

class UCk_Utils_ShapeSphere_UE;

// --------------------------------------------------------------------------------------------------------------------

namespace ck
{
    using FFragment_ShapeSphere_Params = FCk_ShapeSphere_Spec;

    // --------------------------------------------------------------------------------------------------------------------

    struct CKSHAPES_API FFragment_ShapeSphere
    {
    public:
        CK_GENERATED_BODY(FFragment_ShapeSphere);

    public:
        friend class FProcessor_ShapeSphere_HandleRequests;
        friend class UCk_Utils_ShapeSphere_UE;

    private:
        FCk_ShapeSphere_Dimensions _Dimensions;

    public:
        CK_PROPERTY_GET(_Dimensions);

    public:
        CK_DEFINE_CONSTRUCTORS(FFragment_ShapeSphere, _Dimensions);
    };

    // --------------------------------------------------------------------------------------------------------------------

    struct CKSHAPES_API FFragment_ShapeSphere_Requests
    {
    public:
        CK_GENERATED_BODY(FFragment_ShapeSphere_Requests);

    public:
        friend class FProcessor_ShapeSphere_HandleRequests;
        friend class UCk_Utils_ShapeSphere_UE;

    public:
        using RequestType = std::variant<FCk_Request_ShapeSphere_UpdateDimensions>;
        using RequestList = TArray<RequestType>;

    private:
        RequestList _Requests;

    public:
        CK_PROPERTY_GET(_Requests);
    };

    // --------------------------------------------------------------------------------------------------------------------

    CK_DEFINE_SIGNAL_AND_UTILS_WITH_DELEGATE(CKSHAPES_API, OnShapeSphereDimensionsChanged,
        FCk_Delegate_ShapeSphere_OnDimensionsChanged, FCk_Handle_ShapeSphere, FCk_ShapeSphere_Dimensions);
}

// --------------------------------------------------------------------------------------------------------------------