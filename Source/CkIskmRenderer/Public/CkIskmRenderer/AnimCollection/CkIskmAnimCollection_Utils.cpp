...
{
    if (ck::Is_NOT_Valid(InAsset)) { return false; }
    ...
    Get_SequenceFrameIndex(const UCk_IskmAnimCollection_Data* InAsset, int32 InSequenceIndex)
{
    if (ck::Is_NOT_Valid(InAsset)) { return INDEX_NONE; }
    const auto* Baked = InAsset->Get_BakedPose();
    if (Baked == nullptr || !Baked->Sequences.IsValidIndex(InSequenceIndex)) { return INDEX_NONE; }
    ...
    Get_SequenceFrameCount(const UCk_IskmAnimCollection_Data* InAsset, int32 InSequenceIndex)
{
    if (ck::Is_NOT_Valid(InAsset)) { return 0; }
    const auto* Baked = InAsset->Get_BakedPose();
    if (Baked == nullptr || !Baked->Sequences.IsValidIndex(InSequenceIndex)) { return 0; }
