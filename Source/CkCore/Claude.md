...
- **Phase 1 (boot only)** re-runs each AS class's DefaultsFunction chain on its CDO. No pre-reset: scalar/object-ref defaults are idempotent, and clearing first risks abandoning state if a mid-chain statement aborts. It mirrors the engine's `ExecuteDefaultsFunctions` (`ASClass.cpp`) — collect child→parent up the super chain, execute in reverse, each function in its own context so a failure does not skip siblings. **Known limitation:** `default _X.Add(...)` container patterns on CLASS defaults double here — prefer container assignment.
...
- **`Request_ResetToArchetype`.** The script-member copy is a direct call to `asCScriptObject::PerformCopy` — a whole-object assignment of EVERY member declared on the script class, participant included — hence the snapshot/`ON_SCOPE_EXIT` restore of `FCk_Handle_ObjectPoolingParticipant` so binds survive recycling on AngelScript-declared poolables (the contract `CkObjectPoolingParticipant.h` promises). It must NOT go through the engine's registered `UObject::CopyScriptPropertiesFrom` (whose native body is that same `PerformCopy` call): a packaged build booting from a precompiled script cache deletes that registration as unused — no `.as` bytecode calls it — and `ClearUnneededRuntimeData` also erases `allRegisteredTypesByName` (`PrecompiledData.cpp:2536`), so even `GetTypeInfoByName` returns nullptr there. `asCScriptObject` is `ANGELSCRIPTCODE_API`-exported and `ThirdParty/source` is a public include path, so the direct call links fine (the fork's *interface* class `asIScriptObject` is the one that is devirtualized and unexported). Script-only members exist as byte offsets on the fused UObject because the class generator creates `FProperty`s solely for `UPROPERTY`-exported members; a recycled instance would otherwise resume the previous life's state (e.g. a consumed one-shot cursor). The subobject re-instancing mirrors `FObjectInitializer::InstanceSubobjects`; the instancing graph REUSES a same-named per-instance subobject rather than re-creating it, so the reset recurses into each (instance, template) pair. Instanced subobjects inside Set/Map containers are not walked — none exist in the framework today; add the container walk if one ever does. It is public so the contract is directly testable.
