...
| **Player can walk through NPCs** | High (intentional) | NPCs are pure entities with no `UPrimitiveComponent` — `UCharacterMovementComponent` has nothing to collide with, so the player physically passes through any NPC that soft-push didn't displace in time. **This is the design.** Soft-push (separation amplification within `_PlayerProxySoftPushRadius`) is the only mitigation in scope; a hard backstop (small `CkOverlapBody` on each NPC that pushes the player back at very-close range) is **not in scope**. **For testers:** if rental-store playtests report "I walked through a customer," that's the expected behavior — soft-push tuning is the lever. If hard pushback is required, that's a Gate-5+ scope addition (separate effort, ~0.5d). |
