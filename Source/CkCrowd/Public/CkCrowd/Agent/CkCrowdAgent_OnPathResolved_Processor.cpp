...
namespace ck_crowd_agent_on_path_resolved_processor
{
        ...
        -> bool
    {
        ...
        const auto GoalIsOnSurface = Diag.Get_EndProjected();
        const auto Goal = GoalIsOnSurface
            ? Diag.Get_LastProjectedEnd()
            : InPathResult.Get_DestinationLocation();

        return ck::ck_crowd_agent_path_follow_algorithm::Get_RouteEndsShortOfGoal(
            InPathResult.Get_Status(), InPathResult.Get_Waypoints(), Goal, GoalIsOnSurface, InArrivalRadius);
    }
}
...
namespace ck
{
        ...
        -> void
    {
        ...
        {
            const auto StrictEndsShortOfGoal =
