...
## Opt-in CPU work attribution

`ck.Perf.CpuWork 1` enables fixed native CPU scopes and `CkCpuWork_*` counters in trace-capable non-Shipping targets. Default is 0. The mode is latched on the game thread at OnBeginFrame, totals reset then, and OnEndFrame emits the totals across all callers and worlds. Frame/Enabled identify each sample even with work disabled; workload counters emit explicit zeros in enabled frames. Unsupported targets stay disabled. No capture controller, timer, world or actor is owned by this facility.

`CkCpuWork_Frame` is GFrameCounter at emission. Counter timestamps are at frame end, not event start: exclude boundary frames when pairing a partial interval, and never treat a previous enabled value as work in a disabled frame. Counts are work occurrences, not unique entities. Component counters partition valid Record callbacks by the first eligibility failure or changed/unchanged result. Scene child counts are valid entries submitted to Queue, not successful queue insertions. Transform batches include normal/parent-driven drains plus cancellation of undrained requests; force refresh is excluded from parent-driven mutation rejections. Crowd upload candidates count one marshal traversal; the existing velocity-history traversal is additional work. Highlight counts are group push calls. LOD state counts snapshot flags before each entity's work and may overlap; promotion counts are successful transitions, other phase counts are calls.

Names are fixed; callers accumulate in existing traversals and publish bounded totals. The generic Record template, request ordering, component movement, render uploads, and script sweep cadence are unchanged. The AngelScript `ck::CpuWorkScope(uint(ECk_CpuWorkPhase::...))` guard follows the existing late ValueClass binding and balances early returns. On/off capture comparison is required to characterize diagnostic overhead; this instrumentation is not itself a gameplay optimization.
