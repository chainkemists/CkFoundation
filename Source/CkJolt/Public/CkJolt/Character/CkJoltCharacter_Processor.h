...
namespace ck
{
    ...
    // HandleRequests excludes owners already tagged for destruction, so a destroyed character's still-queued
    // requests are never drained. This fires each pending request's completion delegate with
    // Failed_Cancelled so a caller awaiting completion terminates instead of hanging.
    class CKJOLT_API FProcessor_JoltCharacter_CancelPendingRequests : public ck_exp::TProcessor<
        FProcessor_JoltCharacter_CancelPendingRequests,
        FCk_Handle_JoltCharacter,
        ck::TReadOnly<FFragment_JoltCharacter_Requests>,
        CK_IF_END_PLAY>
    {
    public:
    ...
    public:
        ...
        ForEachEntity(
            TimeType InDeltaT,
            HandleType InHandle,
            const FFragment_JoltCharacter_Requests& InRequestsComp)
            -> void;
    };
