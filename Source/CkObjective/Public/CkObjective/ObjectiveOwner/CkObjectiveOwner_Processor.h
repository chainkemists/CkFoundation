...
namespace ck
{
    ...
    // HandleRequests excludes owners already tagged for destruction, so a destroyed owner's still-queued
    // requests are never drained. This fires each pending request's completion delegate with
    // Failed_Cancelled so a caller awaiting completion terminates instead of hanging.
    class CKOBJECTIVE_API FProcessor_ObjectiveOwner_CancelPendingRequests : public ck_exp::TProcessor<
        FProcessor_ObjectiveOwner_CancelPendingRequests,
        FCk_Handle_ObjectiveOwner,
        ck::TReadOnly<FFragment_ObjectiveOwner_Requests>,
        CK_IF_END_PLAY>
    {
    public:
