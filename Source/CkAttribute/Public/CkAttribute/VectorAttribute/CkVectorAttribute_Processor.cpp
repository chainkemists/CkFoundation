...
{
    auto
        FProcessor_VectorAttribute_RetryPendingReplication::
        ForEachEntity(
            TimeType InDeltaT,
            FCk_Handle InHandle,
            FFragment_VectorAttribute_PendingReplicationEntries& InPending) const
        -> void
    {
        for (auto Index = InPending._PendingEntries.Num() - 1; Index >= 0; --Index)
        {
            ...
            auto AttributeEntity = UCk_Utils_VectorAttribute_UE::TryGet(InHandle, Entry.Get_AttributeName());
            if (ck::Is_NOT_Valid(AttributeEntity))
            { continue; }

            ck::attribute::Verbose(TEXT("Retrying pending replication of VECTOR Attribute [{}] to [{}|{}]"),
            ...
            UCk_Utils_VectorAttribute_UE::Request_Override(AttributeEntity, Entry.Get_Base(), Entry.Get_Component());
        ...
        { InHandle.Remove<FFragment_VectorAttribute_PendingReplicationEntries>(); }
    }
}
