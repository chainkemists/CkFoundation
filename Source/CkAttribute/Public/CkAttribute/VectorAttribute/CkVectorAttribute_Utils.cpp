    ...
    -> FCk_Handle_VectorAttribute
{
    ...
    {
        if (const auto* RepData = UCk_Utils_Net_UE::TryGetPendingReplicationData<FCk_RepData_VectorAttributes>(InAttributeOwnerEntity))
        {
            for (const auto& Entry : RepData->Attributes)
            {
                ...
                UCk_Utils_VectorAttributeModifier_UE::Request_ClearAllModifiers(AttributeEntity, Entry.Get_Component());
                Request_Override(AttributeEntity, Entry.Get_Base(), Entry.Get_Component());

                const auto& MaybeModifier = UCk_Utils_VectorAttributeModifier_UE::TryGet(AttributeEntity,
    ...
    -> FCk_Handle_VectorAttribute
{
    CK_ENSURE_IF_NOT(Has_Component(InAttribute, ECk_MinMaxCurrent::Min),
        TEXT("Vector Attribute [{}] does NOT have a [{}] component. Cannot BIND to OnMinClamped"),
        InAttribute,
        ECk_MinMaxCurrent::Min)
    { return InAttribute; }

    CK_SIGNAL_BIND(ck::UUtils_Signal_OnVectorAttributeMinClamped, InAttribute, InDelegate, InBehavior, InPostFireBehavior);
...
{
    CK_ENSURE_IF_NOT(Has_Component(InAttribute, ECk_MinMaxCurrent::Min),
        TEXT("Vector Attribute [{}] does NOT have a [{}] component. Cannot UNBIND from OnMinClamped"),
        InAttribute,
        ECk_MinMaxCurrent::Min)
    { return InAttribute; }

    CK_SIGNAL_UNBIND(ck::UUtils_Signal_OnVectorAttributeMinClamped, InAttribute, InDelegate);
    ...
    -> FCk_Handle_VectorAttribute
{
    CK_ENSURE_IF_NOT(Has_Component(InAttribute, ECk_MinMaxCurrent::Max),
        TEXT("Vector Attribute [{}] does NOT have a [{}] component. Cannot BIND to OnMaxClamped"),
        InAttribute,
        ECk_MinMaxCurrent::Max)
    { return InAttribute; }

    CK_SIGNAL_BIND(ck::UUtils_Signal_OnVectorAttributeMaxClamped, InAttribute, InDelegate, InBehavior, InPostFireBehavior);
...
{
    CK_ENSURE_IF_NOT(Has_Component(InAttribute, ECk_MinMaxCurrent::Max),
        TEXT("Vector Attribute [{}] does NOT have a [{}] component. Cannot UNBIND from OnMaxClamped"),
        InAttribute,
        ECk_MinMaxCurrent::Max)
    { return InAttribute; }

    CK_SIGNAL_UNBIND(ck::UUtils_Signal_OnVectorAttributeMaxClamped, InAttribute, InDelegate);
    ...
    -> FCk_Handle_VectorAttributeModifier
{
    ...
    switch (AttributeComponent)
    {
        case ECk_MinMaxCurrent::Min:
        {
            auto ModifierEntity = VectorAttributeModifier_Utils_Min::Add_Revocable(InAttribute, ParamsToUse.Get_ModifierDelta(), InModifierOperation, InModifierName);
            return ModifierEntity;
        }
        case ECk_MinMaxCurrent::Max:
        {
            auto ModifierEntity =  VectorAttributeModifier_Utils_Max::Add_Revocable(InAttribute, ParamsToUse.Get_ModifierDelta(), InModifierOperation, InModifierName);
            return ModifierEntity;
        }
        case ECk_MinMaxCurrent::Current:
        {
            auto ModifierEntity =  VectorAttributeModifier_Utils_Current::Add_Revocable(InAttribute, ParamsToUse.Get_ModifierDelta(), InModifierOperation, InModifierName);
            return ModifierEntity;
        }
        default:
        {
    ...
    -> FCk_Handle_VectorAttribute
{
    CK_ENSURE_IF_NOT(ck::IsValid(InAttributeModifierEntity),
        TEXT("Trying to Remove and INVALID Vector Attribute Modifier. Is this on a Client and targeting a Replicated Attribute?\n"
