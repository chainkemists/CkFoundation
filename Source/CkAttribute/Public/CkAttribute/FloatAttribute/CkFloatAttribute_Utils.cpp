    ...
    -> FCk_Handle_FloatAttribute
{
    ...
    {
        if (const auto* RepData = UCk_Utils_Net_UE::TryGetPendingReplicationData<FCk_RepData_FloatAttributes>(InAttributeOwnerEntity))
        {
            for (const auto& Entry : RepData->Attributes)
            {
                ...
                const auto& MaybeModifier = UCk_Utils_FloatAttributeModifier_UE::TryGet(AttributeEntity,
