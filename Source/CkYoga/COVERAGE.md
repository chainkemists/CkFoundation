...
| Inspector migration | GOAP Inspector Gateway and Scheduler Processor Inspector successful-path layouts and presentation are fully authored; production data/actions remain in C++ and native builders are startup fallback only. Other debugger inspectors remain to migrate | Open; GOAP Gateway and Scheduler Processor Inspector content complete |
...
Current source supports the completed P9 GOAP Agent List retirement and authored GOAP, Texture and AI outer shells. GOAP's distinct registered ECS Inspector Gateway, Scheduler's processor inspector, and the ECS Relationships inspector are authored; the other debugger inspector surfaces still require migration. Texture Checker Apply/Restore remains native behavior inside its authored page port. Apply the CTO matrix above: remaining shells/inspectors and all-debugger teardown are required work, controller and performance retain bounded gates, while packaging, keyboard accessibility, localization and narrow restacking are deferred and Resource Inspector `+` is excluded.
