...
| Inspector migration | GOAP Inspector Gateway, Scheduler Processor Inspector, ECS Relationships, ECS Entity Info, ECS Network and ECS Path Network successful-path layouts and presentation are authored; production data/actions remain in C++ and native builders are startup fallback/capture paths only. Other debugger inspectors remain to migrate | Open; six inspector slices complete; 43/47 registered ECS inspectors remain native |
