...
| Inspector migration | GOAP Inspector Gateway, Scheduler Processor Inspector, ECS Relationships, ECS Entity Info, ECS Network, ECS Path Network and ECS Tag Set successful-path layouts and presentation are authored; production data/actions remain in C++ and native builders are startup fallback/capture paths only. Other debugger inspectors remain to migrate | Open; seven inspector slices complete; 42/47 registered ECS inspectors remain native |
