...
| Inspector migration | GOAP Inspector Gateway, Scheduler Processor Inspector, ECS Relationships and ECS Entity Info successful-path layouts and presentation are authored; production data/actions remain in C++ and native builders are startup fallback/capture paths only. Other debugger inspectors remain to migrate | Open; four inspector slices complete |
