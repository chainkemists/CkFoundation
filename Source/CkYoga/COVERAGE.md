# Current closeout status after Aggro/Dialog outer-shell reconciliation (2026-09-12)

| Required row | Current evidence | Status |
| --- | --- | --- |
| Authored stable outer shells | GOAP, Texture, AI, EQS, Input, Scheduler, Object Pooling, Style Lab, UI, Aggro and Dialog own stable tabs/splitters/pane composition in authored resources; shared window chrome is authored for its consumers | 11/27 complete; 1 partial; 15 without a full authored shell |
...
| Every-debugger teardown | GOAP, Texture, AI, EQS, Input, Scheduler, Object Pooling, Style Lab, UI, Aggro and Dialog shell teardown plus Scheduler Inspector teardown are focused green examples; the production-host matrix remains incomplete | Open |
...
| CkVisualLodDebugger | `Window/SCkVisualLodDebuggerWindow.cpp`; authored arbiter and crowd tuner resources | Arbiter and dynamic crowd/render-band tuner content is authored, but the alert/tabs/header/stats/overview/tuner/roster/detail/event nested shell remains native. Sole remaining partial outer-shell row. |
