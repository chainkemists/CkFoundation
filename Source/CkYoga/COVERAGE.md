...
| Inspector migration | GOAP Inspector Gateway, Scheduler Processor Inspector, ECS Relationships, ECS Entity Info, ECS Network, ECS Path Network, ECS Tag Set and ECS Objective successful-path layouts and presentation are authored; production data/actions remain in C++ and native builders are startup fallback/capture paths only. Other debugger inspectors remain to migrate | Open; eight inspector slices complete; 41/47 registered ECS inspectors remain native |
