...
The next slice moves all four live summary cards into `AudioDebuggerShell.ui.html/.css`. Incremental Development build and fresh serial D3D12 `Ck.AudioDebugger.AuthoredShell` passed 1/1 in `scratch/yoga-audio-authored-stats-final3-20260914.log` (SHA256 `05AE848034CB1CEE6C1401A7E89B4FF4CA7D6CDB27C27ABCAD547C20C082D083`), proving authored labels, native-subtree exclusion, `0 / 0` to `1 / 4` projection, compatible/rejected reload and native-fallback recovery.

This is not Audio acceptance and does not change the 14/27 census. Tabs, filters and six ordinary page presentations remain native, Radar/FalloffCurve still need retained adapters, and the broader production-host every-debugger teardown matrix remains open.
