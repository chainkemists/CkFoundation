...
public:
    UPROPERTY() ECk_AnimationToolbox_ScanScope Scope = ECk_AnimationToolbox_ScanScope::AnimBlueprintDeps;

    /** When Scope == AnimBlueprintDeps */
    UPROPERTY() TSoftObjectPtr<UAnimBlueprint> AnimBlueprint;

    /** When Scope == Path */
    UPROPERTY() FString ContentPath = TEXT("/Game/");

    /** Frame-0-to-1 pose delta must exceed this multiple of the median transition delta to flag the anim. */
    UPROPERTY() float OutlierMultiplier = 3.0f;
    ...
    UPROPERTY() bool bRequireLoopBoundaryMismatch = true;
};
