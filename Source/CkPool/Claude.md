...
`_GrowBatchCount` (1 = demand-exact growth; higher values TOP UP queued extras to GrowBatchCount-1
through the amortized prewarm budget — a batch never hitches the frame, a burst of misses provisions
one batch not one per miss, and subsequent misses become hits).
