    ...
    -> ECk_Inventory_OperationResult_Stack
{
    ...
    // Only checkable with a valid inventory: callers that pass an invalid one to skip the
    // containment check (e.g. Get_StackRoomFor) apply the StackingPolicy clamp themselves.
    if (ck::IsValid(InInventory)
    ...
    -> FCk_FillExistingStacksResult
{
        ...
        const auto& ExistingItem : ExistingItems)
    {
        ...
        // Same-definition items can still be unstackable when a trait distinguishes runtime state.
        // InSourceItem is invalid for definition-driven fills — gate only when it is valid.
        if (ck::IsValid(InSourceItem) && NOT InDefinition->CanStackWith(InSourceItem, ExistingItem))
        { continue; }

        // InSourceItem may be invalid here; custom validators must tolerate that.
