...
//
// Convention under test (documented in CkCompass_Math.h): heading/bearing are world yaws in degrees,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FCkTest_Compass_Math_SignedBearing_CardinalOffsets,
    "Ck.CkCompass.Math.SignedBearing_CardinalOffsets",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FCkTest_Compass_Math_SignedBearing_CardinalOffsets::RunTest(const FString&)
{
    const auto Observer = FVector::ZeroVector;

    // Heading 0 (facing +X / North)
    TestTrue(TEXT("target dead ahead -> 0"),
        FMath::IsNearlyEqual(ck::compass::Get_SignedBearingDegrees(Observer, FVector{100, 0, 0}, 0.0f), 0.0f, 0.01f));
    TestTrue(TEXT("target East (+Y) -> +90"),
        FMath::IsNearlyEqual(ck::compass::Get_SignedBearingDegrees(Observer, FVector{0, 100, 0}, 0.0f), 90.0f, 0.01f));
    TestTrue(TEXT("target West (-Y) -> -90"),
        FMath::IsNearlyEqual(ck::compass::Get_SignedBearingDegrees(Observer, FVector{0, -100, 0}, 0.0f), -90.0f, 0.01f));
    TestTrue(TEXT("target behind -> |180|"),
        FMath::IsNearlyEqual(FMath::Abs(ck::compass::Get_SignedBearingDegrees(Observer, FVector{-100, 0, 0}, 0.0f)), 180.0f, 0.01f));

    // Heading 90 (facing +Y / East)
    TestTrue(TEXT("heading 90, target +Y -> 0"),
        FMath::IsNearlyEqual(ck::compass::Get_SignedBearingDegrees(Observer, FVector{0, 100, 0}, 90.0f), 0.0f, 0.01f));
    TestTrue(TEXT("heading 90, target +X -> -90"),
        FMath::IsNearlyEqual(ck::compass::Get_SignedBearingDegrees(Observer, FVector{100, 0, 0}, 90.0f), -90.0f, 0.01f));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FCkTest_Compass_Math_SignedBearing_WrapAround,
    "Ck.CkCompass.Math.SignedBearing_WrapAround",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FCkTest_Compass_Math_SignedBearing_WrapAround::RunTest(const FString&)
{
    ...
    TestTrue(TEXT("heading 350 vs target yaw 10 -> +20"),
        FMath::IsNearlyEqual(ck::compass::Get_SignedBearingDegrees(Observer, Get_LocationAtWorldYaw(10.0f), 350.0f), 20.0f, 0.01f));

    TestTrue(TEXT("heading 10 vs target yaw 350 -> -20"),
        FMath::IsNearlyEqual(ck::compass::Get_SignedBearingDegrees(Observer, Get_LocationAtWorldYaw(350.0f), 10.0f), -20.0f, 0.01f));

    TestTrue(TEXT("heading -10 (denormalized) vs target yaw 10 -> +20"),
        FMath::IsNearlyEqual(ck::compass::Get_SignedBearingDegrees(Observer, Get_LocationAtWorldYaw(10.0f), -10.0f), 20.0f, 0.01f));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FCkTest_Compass_Math_NormalizedArcOffset_ClampEdges,
    "Ck.CkCompass.Math.NormalizedArcOffset_ClampEdges",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FCkTest_Compass_Math_NormalizedArcOffset_ClampEdges::RunTest(const FString&)
{
    constexpr auto Arc180 = 180.0f;

    TestTrue(TEXT("bearing 0 -> offset 0"),
        FMath::IsNearlyEqual(ck::compass::Get_NormalizedArcOffset(0.0f, Arc180), 0.0f, 0.001f));
    TestTrue(TEXT("bearing 45 in arc 180 -> 0.5"),
        FMath::IsNearlyEqual(ck::compass::Get_NormalizedArcOffset(45.0f, Arc180), 0.5f, 0.001f));
    TestTrue(TEXT("bearing 90 in arc 180 -> exactly the edge (1.0)"),
        FMath::IsNearlyEqual(ck::compass::Get_NormalizedArcOffset(90.0f, Arc180), 1.0f, 0.001f));
    TestTrue(TEXT("bearing -90 in arc 180 -> -1.0"),
        FMath::IsNearlyEqual(ck::compass::Get_NormalizedArcOffset(-90.0f, Arc180), -1.0f, 0.001f));
    TestTrue(TEXT("bearing 135 in arc 180 -> clamped to 1.0"),
        FMath::IsNearlyEqual(ck::compass::Get_NormalizedArcOffset(135.0f, Arc180), 1.0f, 0.001f));

    // Edge-inclusive arc test: exactly the half-arc is INSIDE; beyond it is outside
    TestFalse(TEXT("bearing 90 is NOT outside arc 180"), ck::compass::Get_IsOutsideArc(90.0f, Arc180));
    TestTrue(TEXT("bearing 90.1 IS outside arc 180"), ck::compass::Get_IsOutsideArc(90.1f, Arc180));
    TestTrue(TEXT("bearing 60 IS outside arc 90"), ck::compass::Get_IsOutsideArc(60.0f, 90.0f));
    TestFalse(TEXT("bearing -30 is NOT outside arc 90"), ck::compass::Get_IsOutsideArc(-30.0f, 90.0f));

    // Degenerate arc never divides by zero
    TestTrue(TEXT("zero arc -> offset 0, no div-by-zero"),
        FMath::IsNearlyEqual(ck::compass::Get_NormalizedArcOffset(45.0f, 0.0f), 0.0f, 0.001f));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FCkTest_Compass_Math_CardinalAndOrdinal_Buckets,
    "Ck.CkCompass.Math.CardinalAndOrdinal_Buckets",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FCkTest_Compass_Math_CardinalAndOrdinal_Buckets::RunTest(const FString&)
{
    using ck::compass::Get_CardinalAndOrdinalDirection;
