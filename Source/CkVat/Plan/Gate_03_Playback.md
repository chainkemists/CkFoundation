...
After this gate: `UCk_Utils_Vat_UE::Add` + `Request_PlayClip` on a Transform-bearing entity renders
...
- `FProcessor_Vat_Setup`: initial clip state (+ `RandomPerInstance` phase offset for crowd variety),
  then composes the IsmProxy on the SAME entity (requires a Transform — loud ensure otherwise) and
  pushes the initial 12 floats.
- `FProcessor_Vat_HandleRequests`: one `Request_SetCustomInstanceData` push per drained batch.
- `FProcessor_Vat_FireSignals`: CPU mirror of the GPU clock for Once clips → `OnClipFinished`
...
- Float packing (`ck_vat_processor::Pack_CustomData`) ⇄ look per-instance declaration order ⇄
...
1. In a test map/gym: spawn an entity with Transform + `UCk_Utils_Vat_UE::Add(collection, InitialClip)`.
