...
| ~~Bone mode: multi-influence regions shred into spikes, single-influence regions (feet) fine~~ **HIT + FIXED 2026-07-10** | Static-mesh build sRGB-encodes vertex-color weights (`ToFColor(true)`); blended weights warp upward (sums ~1.5) | baker now pre-decodes weights (`SrgbToLinear` in `BuildBakedStaticMesh`) — if it recurs, check the build's color conversion changed |
...
| ~~Whole mesh rigidly folded onto one bone, identical across instances, sometimes per-run random~~ **HIT + FIXED 2026-07-10** | MID `BoneCount`/`TotalRows` seeded 0: `GetSizeX/Y()` reads PLATFORM data = 0 while a fresh bake's textures still async-compile (race ⇒ nondeterminism); shader lookup wraps onto one texel | dims now SERIALIZED (`_TextureWidth/_TextureRows`) and the MID seeds from them. Diagnose any future VAT visual bug with `Ck_Vat_DebugVerifyBake` FIRST (all-PASS ⇒ material/VF layer). Also: never debug visuals through stacked Live Coding patches — clean rebuild first |
