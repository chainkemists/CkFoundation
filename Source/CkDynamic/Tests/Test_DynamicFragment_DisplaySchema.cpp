...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    ...
    "Ck.PackagedEnsure.CkDynamic.DisplaySchema.WorkerThreadRefreshMarshalsToGameThread",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
#endif
...
bool FCkTest_DynamicFragment_DisplaySchema_WorkerThreadRefresh::RunTest(const FString&)
{
    ...
    // A sentinel generation no observed path can produce: its disappearance proves the refresh ran.
    ...
    TestTrue(TEXT("the refresh request is issued off the game thread"), RequestRanOnWorker.load());
    TestEqual(TEXT("the worker request fires no ensure"),
    ...
    TestFalse(TEXT("the game-thread ticker pump replaced the AS generation"),
        ck::dynamic::TryGet_FragmentDisplaySchema(StaleAsPath, Stored));
