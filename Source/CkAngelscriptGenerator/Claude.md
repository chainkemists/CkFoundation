...
The canonical files are never touched. Both the BB root `.gitignore` and CkFoundation's `.gitignore` cover `Script/Generated/_StubRecovery_*` patterns, so stub files physically cannot be staged. Cleanup runs from TWO hooks:

* **`StartupModule`** — walks all enabled `Script/Generated/` directories at module load (before AS delegates are wired) and deletes any matching stale files. Catches force-quit survivors before they can collide with regenerated canonical output. Added 2026-05-14 after a user-reported `_StubRecovery_BusterBlockAssets.as` survived to next launch and tripped a duplicate-function compile error.
* **`OnPostCompile`** — same walk, runs on every successful AS compile. Self-cleaning during normal operation.
