...
- `CkAngelscriptGenerator_AsErrorParser.{h,cpp}` — parses Hazelight's AS compile-error output (sourced via `FAngelscriptManager::Get().FormatDiagnostics()`) into typed root-cause records. Recognizes three error classes: `No matching signatures to '<NS>::<func>(<args>)'`, `Identifier '<X>' is not a data type`, and `Instead found '<string constant>'` (the unique signature of a `"foo " "bar"` C-style adjacent-literal splice — AS rejects, Hazelight emits as a two-line pair with the generic `Expected ')' or ','` companion that the parser ignores). Cascade noise (`Unknown`, `_Iterator`, etc.) is dropped.
...
- `CkAngelscriptGenerator_Dispatcher.{h,cpp}` — classifies parsed roots into one of four recovery strategies and applies them.
...
The canonical files are never touched. Both the BB root `.gitignore` and CkFoundation's `.gitignore` cover `Script/Generated/_StubRecovery_*` patterns, so stub files physically cannot be staged. Cleanup is **per-generator** and runs at three different sites:

* **`StartupModule`** — walks all enabled `Script/Generated/` directories at module load (before AS delegates are wired) and deletes ANY `_StubRecovery_*` file via the broad `Delete_AllStubRecoveryFiles()` sweep. Catches force-quit survivors before they can collide with regenerated canonical output. Added 2026-05-14 after a user-reported `_StubRecovery_BusterBlockAssets.as` survived to next launch and tripped a duplicate-function compile error.
* **`OnPostCompile` (synchronous part)** — after the synchronous generators have run (`Run_AllGenerators` for EntitySpawnParams, `Maybe_RegenDynamicHandleJson_OnPostCompile` for the JSON), the lambda deletes `_StubRecovery_*_EntitySpawnParams.as` + `_StubRecovery_*.json` via `Delete_StubRecoveryFiles_ForPatterns(...)`. The **AssetRegistry sibling pattern is intentionally excluded here** — see below.
* **AssetRegistry FTSTicker post-regen** — inside `Maybe_RegenAssetRegistry_OnPostCompile` (and its `OnPostInit` twin), immediately after `Subsystem->GenerateAllAssetRegistries()` returns and BEFORE clearing the `sg_AnyAssetRegistryRegenInFlight` flag, the ticker callback deletes `_StubRecovery_*Assets.as` via the pattern-scoped sweep. This is the contract that fixes the loop pinned by `CkAuto/SelfHealProbes/_probe_assetregistry_loop`: the AR ticker is async (idle-wait + `InDelay=2.0f`), so deleting the AR sibling in the outer synchronous PostCompile lambda races the regen — Hazelight's hot-reload sees the deletion mtime change, recompiles against the still-stale canonical, fails the same root, the dispatcher re-synthesizes, and the loop continues indefinitely (observed 51+ post-regen cycles in the captured red baseline). AR cleanup must wait until the canonical has actually been rewritten.

**Per-accessor dedup gate:** Both synthesizers (`Inject_EntityScriptParamsStub` and `Inject_AssetRegistryStub`) scan existing sibling content for the unique `// End synthesized stub for <NS>::<FUNC>` marker line before appending. If present, the append is a no-op (returns success). Without this, a dispatcher re-fire for the same accessor within one session produces duplicate `Params(...)` / accessor declarations in the merged namespace, which AS rejects with `A function with the same name and parameters already exists`. Regression test: `CkAngelscriptGenerator.UnitTests.StubSynthesizer.Inject_DedupOnSameAccessor`.

**EntitySpawnParams stub:** AS merges multiple `namespace X { ... }` blocks across files, so the sibling's namespace block contributes the missing accessor to the merged scope and compile succeeds. The sibling carries a recovery-header banner (`FCkAsStubSynthesizer::Get_StubFileHeader()`) plus per-stub marker comments (`Get_MarkerComment()`). Multiple drifts for distinct accessors in the same session accumulate into the same sibling file; same-accessor re-injects are dedup-gated.
...
**Force-quit between recovery and PostCompile:** Sibling files survive to next launch. AS compile succeeds again on next launch (the sibling is still on disk, merge-via-namespace still works, the JSON merge still works). `StartupModule`'s broad `Delete_AllStubRecoveryFiles()` sweep — fired before AS delegates are wired — then deletes any leftover stubs unconditionally. The `_StubRecovery_*` files double as the unambiguous on-disk signal for the deferred-regen paths (`Maybe_RegenAssetRegistry_OnPostInit`, `Maybe_RegenDynamicHandleJson_OnPostInit`) to know they should fire a proper regen — replacing the marker-scan-inside-canonical approach from the prior in-place model.

**Per-generator cleanup invariant (post-2026-05-21):** Each `_StubRecovery_*` pattern is owned by the generator that emits its canonical, and cleanup happens AFTER that generator has finished. The `Delete_StubRecoveryFiles_ForPatterns(TArrayView<...>)` helper in `CkAngelscriptGenerator_Module.cpp` is the only function that should be called from non-startup contexts — pass the specific pattern(s) tied to the generator that just completed. Calling `Delete_AllStubRecoveryFiles()` (the no-arg broad sweep) outside `StartupModule` reintroduces the AR loop bug because the AR regen is async. The unit-test surface here is the `_probe_assetregistry_loop` probe rather than a C++ test, since the race involves Hazelight's hot-reload thread + FTSTicker timing that can't be cleanly mocked.
