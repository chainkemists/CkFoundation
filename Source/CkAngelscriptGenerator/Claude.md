...
**Lifetime semantics survive regeneration, and strong retention is deliberately asymmetric.** Weak/soft wrappers (`TWeakObjectPtr` / `TSoftObjectPtr` / `TSoftClassPtr`, including inside `TArray`) are lifetime semantics, not type spelling — `Get_DetailedPropertyType` preserves them verbatim across both the fields and both `Params()` signatures. A *strong* `TObjectPtr<T>` is treated differently on purpose: the `Params()` call keeps the source strong type, while `Get_RetainedPropertyType` weakens direct strong retention to `TWeakObjectPtr<T>` in the EntitySpawnParams struct — the params blob must not root the object. Pinned by `…ParamsGenerator.PreservesWeakAndSoftObjectWrappers` + `.InjectsWeakMirrorIntoStrongOwner`.
