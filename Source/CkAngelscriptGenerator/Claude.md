...
1. **Generators** — write the canonical generated `.as` / `.json` accessor files from live reflection + asset state.
   ...
   - `FCkAutoTestNetStubGenerator` — net-test `.spec.cpp` stubs, one file per feature, routed into the repo that owns the `.as` test. See *Net-test stub generator* below.
...
**`-CkAsDeclineRegenOwnership` — read-only boots decline the lock.** A boot that will never legitimately write (the toolbox's `Automation List; Quit` inline-discovery pass) passes this flag and declines ownership at every gate; generator writes and self-heal are inert in the declining process. Without it, the discovery boot can WIN the lock in the `--build --test` race, demoting the REAL test editor to a secondary with self-heal inert — which can then execute stale bytecode ("Keeping all old script code") and report a toolbox false-green. Declining leaves the lock free for the test editor to own and self-heal as PRIMARY.
...
- `CkAngelscriptGenerator_AsErrorParser.{h,cpp}` — parses Hazelight's AS compile-error output (sourced via `FAngelscriptManager::Get().FormatDiagnostics()`) into typed root-cause records. Recognizes five error classes: `No matching signatures to '<NS>::<func>(<args>)'`, `No matching signatures to '<Ident>(<args>)'` (bare constructor-style call — no `::`; the direct-construction shape of a missing `F<X>_SpawnParams`), `Identifier '<X>' is not a data type`, `'<Member>' is not a member of '<Struct>'` (a field deleted out from under a stale ESP canonical; the `'<Member>' is not a member of 'Unknown'` spelling stays cascade noise and is dropped explicitly — promoting it would let an unrelated author error trigger a canonical DELETE), and `Instead found '<string constant>'` (the unique signature of a `"foo " "bar"` C-style adjacent-literal splice — AS rejects, Hazelight emits as a two-line pair with the generic `Expected ')' or ','` companion that the parser ignores). Cascade noise (`Unknown`, `_Iterator`, `'<Field>' is not a member of 'Unknown'`, etc.) is dropped.
...
#### Source scanner — cache + known limits

`FCk_AsSourceScanCache` exists because a self-heal drain resolves one class shape per drift, and one per class during a quarantine resynthesis. Uncached, each call re-runs the full recursive `*.as` enumeration AND cold-re-reads files until the class matches, over the same unchanging on-disk tree; threading one cache through a drain collapses N tree walks plus redundant reads into a single walk and one read per file. (Its lifetime contract stays in the header.)

Known textual-parse limitations — full fidelity belongs to the real generator: `#if` blocks inside class bodies are parsed as-is, and multicast (event) delegate members are not excluded. Neither exists on `ExposeOnSpawn` entity-script properties in practice, and a shape mismatch self-corrects via the per-signature error-text fallback path.

#### Class-resolver tiers — what each one buys

Cheapest viable first; callers pick the tiers they need (the shared-tiers invariant and the walk-past-BPGC rule are above).

| Tier | Mechanism | Works when |
|---|---|---|
| `Resolve_ViaLoadObject` | `LoadObject` + native-parent walk | modal-tick, for any asset whose native class module is loaded |
| `Resolve_ViaAssetDataTag` | AssetRegistry point-query (`GetAssetByObjectPath` — bypasses the `IsEngineStartupModuleLoadingComplete` gate) reading `NativeParentClassPath`, falling back to `ParentClassPath` | no class-load possible; fails when AR cached `"None"` (common for AS-parented WBPs) |
| `Resolve_ViaPackageReader` | direct `.uasset` linker-table walk, AssetRegistry-free | AR's cache is poisoned AND the asset can't be constructed. Path A reads the generated-class export's `SuperIndex` (zero'd when the `.uasset` was saved with an unloadable parent); Path B scans the import table for Class-typed AS imports, tie-breaking multiple candidates on longest shared prefix with the asset basename |

### Recovery strategies

| Error pattern | Strategy | Wired? |
|---|---|---|
| `<EntityScriptClass>::Params(<args>)` not found | **Synthesize stub** into a sibling `_StubRecovery_<Plugin>_EntitySpawnParams.as` — AS merges the sibling's namespace block with the canonical's, finds the missing overload, compile succeeds. `OnPostCompile` deletes the sibling file after a successful regen. Prefers the **source-derived full shape** (below); falls back to the error-text stub when the class source can't be found or when an earlier error-text stub in the sibling owns the struct (in-session incremental drift). | ✓ |
| `<EntityScriptClass>::Params(<args>)` not found, struct defined in a PRESENT canonical (**Rev 11 stale-canonical quarantine**) | The canonical exists but its signatures drifted from the source — e.g. an untracked leftover surviving a pull after the gitignore flip. The per-signature error-text path cannot heal mixed-type callers against it (error-text stubs are typed from ONE call site; AS has no base→typesafe handle conversion; the same-arity ambiguity gate then blocks every further overload — the 2026-06-11 wedge: green toast, red compile). Escalation: `Quarantine_And_ResynthesizeFullShapes` — forensic copy of the canonical to `Saved/CkSelfHeal/Quarantine/`, DELETE the canonical (it regenerates on the next successful compile), drop the sibling, then bulk source-derived full shapes for the union of classes the canonical covered. Also triggered when the error-text path reports `SameArityAmbiguous` (which no longer fakes success). | ✓ |
...
| `Identifier '<X>' is not a data type` **inside a `*_EntitySpawnParams.as` canonical**, where `<X>` is neither `FCk_Handle_*` nor `F<X>_SpawnParams` (**deleted-field-type stale-canonical**, 2026-07) | The canonical references a type — typically an `ExposeOnSpawn` field enum/struct — that was renamed or deleted out from under the stale, machine-local canonical after a pull (e.g. `EBb_NamedNpc`/`EBb_Employee` unified into `EBb_Npc`). The error is located *inside* the canonical, names no entity-script class, and matches neither the handle nor the SpawnParams shape, so the Rev 11 quarantine (keyed on a caller's `::Params` signature drift) never sees it — pre-fix this fell through to the terminal banner and wedged the boot. `Classify` routes it to `Quarantine_StaleEspCanonical` **keyed on the error's LOCATION** (`Is_EntitySpawnParamsCanonicalPath` — the `_EntitySpawnParams.as` suffix under `Script/Generated/`, the gitignored delete-safe family). Apply calls the same **seedless** `Quarantine_And_ResynthesizeFullShapes(canonical, {}, …)` — the class union is enumerated from the canonical's own `namespace U<X>` blocks, so every shape it covered is rebuilt from current source in one pass. One-shot per canonical per phase (`sQuarantinedEspCanonicals`, no MaxCycles mid-session). The identical error in **author source** or a **tracked** generated file (`BusterBlockAssets.as`, `*_AutoTestActors.as` — neither carries the `_EntitySpawnParams.as` suffix) stays a terminal banner (a real authoring bug / never delete tracked state). Regression: `Dispatcher.Classify_StaleEspCanonical`. | ✓ |
| `'<Member>' is not a member of '<Struct>'` **inside a `*_EntitySpawnParams.as` canonical** (**deleted-FIELD stale-canonical**, 2026-08) | The deleted-**field** twin of the row above, and the everyday shape: a non-default `ExposeOnSpawn` params field is emitted as a field-assignment in the SpawnParams default ctor, so deleting the field strands the assignment in every machine-local canonical generated before the deletion. `git pull` never touches the gitignored canonical and the compile dies before `OnPostCompile` can rewrite it — permanent wedge, boot after boot, for everyone who had a pre-deletion file on disk (a fresh clone is fine; the author is immune because their editor regenerated across the change). Pre-fix the error matched none of the four recognized shapes, so the boot ended at `Parsed 0 actionable roots` → terminal banner. `Classify` routes it to `Quarantine_StaleEspCanonical` on the **same LOCATION predicate** as the 2026-07 row, reusing the seedless `Quarantine_And_ResynthesizeFullShapes` unchanged. Reproduced end-to-end 2026-08-07 (`LocalRotationOffset` dropped from `FBb_Fragment_OpenSign_ParamsData` by `2647ca2cf`). Regressions: `AsErrorParser.NotAMemberOfStructProbe` (verbatim FormatDiagnostics capture — the modal's rendering differs and must never be the fixture source), `AsErrorParser.NotAMember_CascadeAndPerCanonicalDedup`, `Dispatcher.Classify_StaleEspCanonical_DeletedField`. | ✓ |
...
**Drain routing.** Bootstrap defers via `FSlateApplication::GetOnModalLoopTickEvent`; MID-SESSION hot-reload failures never open a modal, so the drain is an FTSTicker instead. `Is_BootstrapMode` picks between them and is flipped by the Module's `OnFEngineLoopInitComplete` lambda.

**Headless is the exception — drains apply synchronously inside the broadcast.** Commandlet/unattended runs (`Drain_PendingActions_Headless`) must write during `OnReloadHadErrors` itself: Hazelight hard-exits commandlets immediately after the broadcast (`Cannot run when angelscript has failed to compile`, the `IsRunningCommandlet` guard in `AngelscriptManager.cpp`), so that broadcast is the only window. It is also SAFE there — that flow has no hot-reload watcher whose mtime baseline could swallow the writes. The stubs land before the exit and the next run of the same command compiles against them. Pinned by the 2026-06-10 CI cook failure on the first gitignored-EntitySpawnParams fresh clone.

**In-progress toast orphan guard (5s).** If modal-tick hasn't transitioned the notification within 5s, the failure path that fired `OnReloadHadErrors` opened no Hazelight modal — the typical "Cycle 2 with no modal" hot-reload retry after a successful initial-compile recovery (2026-05-13). Cycle 1 transitions in ~1.2s empirically, so 5s sits comfortably outside the normal path.
...
Hazelight's AS compile-error output is **not a stable public API**. The parser's regex patterns are derived from empirical capture of probe formats (2026-05-12 for the original three drift-recovery formats; 2026-05-17 for the adjacent-string-literal pattern; the bare-ctor `No matching signatures to '<Ident>(<args>)'` snapshot came from the 2026-06 fresh-clone ESP boot test, NOT the 2026-05 corruption probes). Snapshot tests under `Tests/Test_AsErrorParser.cpp` pin every known-good format verbatim — they are the engine-upgrade canary. If those tests fail after a Hazelight upgrade, the format has changed and the parser must be updated before the dispatcher is re-enabled.
...
## AutoTestActors wrapper generator — output convention + author opt-out
...
## Net-test stub generator — three-destination routing

`FCkAutoTestNetStubGenerator` emits one `.spec.cpp` per **feature**, derived from the AS source path by `Derive_FeatureFromSourcePath` (plugin convention `Script/Ck<Feature>/`, project convention `Script/Tests/<Feature>/`; falls back to `AS`; the file's own name is never a feature dir, and the convention-matching segment NEAREST the leaf wins — so `Script/CkFoo/Tests/` stays plugin-convention). Destinations:

1. **Project-authored test** (under `<ProjectDir>/Script/`) → `<ProjectDir>/Source/<ProjectName>/Tests/Net/Generated/<Feature>_NetAutoTestStubs.spec.cpp`.
2. **Test in a plugin other than CkTests that ships a compiled C++ module** → `Plugins/<X>/Source/<Module>/Private/Net/Generated/…`, where `<Module>` is the plugin's first Runtime module, else UncookedOnly/DeveloperTool. Editor-typed modules are skipped — the file must still compile in the editor target's build.
3. **Everything else** — CkTests' own net tests, or a script-only plugin — falls back to `Plugins/CkTests/Source/CkTests/Private/Net/Generated/…`.

**The split is load-bearing, not cosmetic.** These stubs are COMMITTED (UBT must compile them before the editor can boot to regenerate them) and the generator actively prunes stale files, so a stub MUST live in the same repo as the `.as` class it references — git has to keep the pair atomic across branch switches. Before the split, a project-authored test's stub landed in the CkTests submodule and was pruned on every machine whose checkout lacked the class, surfacing as a mysterious submodule deletion every boot; routing a plugin's stub into CkTests recreates that churn for the plugin's repo. This is why `Get_IsProjectAuthoredPath` must not classify a plugin test as project-authored merely because `Plugins/` physically nests under `<ProjectDir>/`. Pinned by `CkAngelscriptGenerator.UnitTests.NetStubGenerator.*`.

Whichever destination wins, the HOSTING module must publicly depend on `CkTests` — the emitted C++ includes `CkTests/Net/CkAutoTest_NetSubject.h` + `CkNetAutomation_Common.h` and resolves `/Script/CkTests.Ck_AutoTest_NetSubject`. The generator itself needs no CkTests build dependency: `_NetMode` / `_TimeoutSeconds` are read off the CDO via reflection. Triggers and determinism guarantees match the wrapper generator (OnPostEngineInit + every successful AS PostCompile; stable sort, no timestamps, LF-normalized diff-skip, CRLF write on Windows).

**Incident — the default NetSubject class path carries no `A` prefix.** UE's `FSoftClassPath` C++-class lookup STRIPS the `A`/`U`/`F` prefix, so `DefaultNetSubjectClassPath` must be `/Script/<Module>.<Name-without-prefix>`. Until that was fixed, every AS net test that did NOT override `_NetSubjectClass` failed at `SpawnActor` with "failed to resolve NetSubject class via FSoftClassPath" (`TryLoadClass` returns nullptr on the prefixed form). The per-test override path is immune — it uses `Resolved->GetPathName()`, already stripped. The first net tests (CkAttribute) happened to override the path, so the bug hid until CkTransform, which uses the default subject, tripped it. Related: the default `ACk_AutoTest_NetSubject` only adds a Float attribute on its entity script — modules needing different per-entity state (CkRelationship, CkInventory, CkStateMachine) author a subclass and override the UPROPERTY on their AS test class.

---

## AssetRegistry regen — scheduling contract

**Cold start hangs off `OnFEngineLoopInitComplete`, NOT `OnPostEngineInit`** (`Maybe_RegenAssetRegistry_OnPostInit`). The latter fires INSIDE `FEngineLoop::Init`'s slow task, so AR opening its own `FSlowTask` trips an out-of-order `FSlowTask` destruction ensure (caught 2026-05-12). Inside the callback an FTSTicker polls until BOTH the shader compiler is idle AND AR has finished cataloging. Without the AR-idle gate, `GenerateAllAssetRegistries` can fire mid-scan and emit partial `*Assets.as` with accessors silently missing; without the shader gate, ~400 texture async loads pile onto cold-start contention (6.5min wedge vs a normal 3.7s pass, 2026-05-12). Hard cap 30 polls (~60s) — fires anyway with a Warning rather than hanging.

**PostCompile regen must be FTSTicker-deferred, never synchronous.** PostCompile broadcasts from inside `FAngelscriptManager::CompileModules`'s `FSlowTask` scope ("Script Module Compilation"); `GenerateAllAssetRegistries` opens its own, and a synchronous call leaves the AR slow task on the stack when CompileModules's destructs → the `Task==this` ensure at `SlowTask.cpp:149` (caught 2026-05-13). The DynamicHandle counterpart CAN be synchronous for the same reason inverted: it only reads already-loaded data assets and writes a small JSON.

**Rejected design — no `OnAssetRegistryComplete` subscription.** Double-firing between the cold-start and PostCompile paths is prevented by a shared in-flight bool plus the on-disk sibling-file scan, not by subscribing to that delegate — it is a UFUNCTION dynamic multicast, i.e. a whole UCLASS sink for one callback. The file-scan marker is cheaper.

**Stub discovery is AssetRegistry-FREE by necessity.** At modal-tick during initial-compile failure the engine is mid-startup-module-loading, so AR's `SearchAllAssets`/`GetAssetsByClass` exit early on `IsEngineStartupModuleLoadingComplete() == FALSE` — empirically the first wired AR call returned 0 `UCkAssetRegistryConfig` assets even after `Force_FullArScan` (2026-05-12). Replacement: (1) file-scan `Script/Generated/*Assets.as` across project + plugins and parse the `// Discovery root:` header; (2) locate the asset on disk via `FPackageName::TryConvertLongPackageNameToFilename` + `IFileManager::FindFilesRecursive` for `<FunctionName>.uasset`. Note the generated header's `// Source config:` line carries a malformed path token (missing slash between root and filename — a U+2192 arrow concat bug fixed after `c408ee8be`, but stale files persist), which is why `Try_ParseConfigSiteHeader` reads the namespace from that line but takes the root from `// Discovery root:`.
...
## EntitySpawnParams emission rules

**Struct-typed `Params` fields containing a `UObject*` are emitted without an inline initializer.** When a struct-typed field's CDO differs from the struct default, the naive emission is a positional-ctor expression as the field initializer (`<Type> Params = <Type>(arg1, …, nullptr);`). For any struct carrying a `UObject*` field, AS overload resolution rejects the bare `nullptr` arg as `<null handle>` and the canonical becomes unparseable. The generator therefore declares the field with NO inline initializer and moves per-field overrides into the SpawnParams default-ctor body, where the assignment is in *statement* context — the LHS slot types the RHS, so bare `nullptr` binds exactly as an in-struct field default does. Gate helper `UCk_Utils_Reflection_UE::Has_UObjectPointerField`; override list `Get_StructFieldOverrides` (dotted leaf paths). Pinned by `…ParamsGenerator.Has_UObjectPointerField` / `.Get_StructFieldOverrides_*`.

**Lifetime semantics survive regeneration, and strong retention is deliberately asymmetric.** Weak/soft wrappers (`TWeakObjectPtr` / `TSoftObjectPtr` / `TSoftClassPtr`, including inside `TArray`) are lifetime semantics, not type spelling — `Get_DetailedPropertyType` preserves them verbatim across both the fields and both `Params()` signatures. A *strong* `TObjectPtr<T>` is treated differently on purpose: the `Params()` call keeps the source strong type, while `Get_RetainedPropertyType` weakens strong retention to `TWeakObjectPtr<T>` in the EntitySpawnParams struct — the params blob must not root the object. Pinned by `…ParamsGenerator.PreservesWeakAndSoftObjectWrappers` + `.InjectsWeakMirrorIntoStrongOwner`.

**Weakening recurses into containers, and containers take the retained type in `Params()` too.** `Get_RetainedPropertyType` walks `TArray` / `TSet` / `TMap` / `TOptional` and weakens the strong UObject LEAF in place (`TArray<TObjectPtr<T>>` → `TArray<TWeakObjectPtr<T>>`), mirroring `Get_DetailedPropertyType`'s own container walk. This is not cosmetic: `ck::Analyze_UntracedStructSafety` walks into containers, so a raw `TArray<UObject>` field is REJECTED at `Request_SpawnEntity` / `Request_ExecuteCue` with `raw/strong UObject leaf [ObjectProperty] requires a GC-traced owner` — the scalar was weakened and accepted while its container twin ensured at runtime (Venus, 2026-08: `Vns_WeaponFireCue_SpawnParams.Sounds[]` and `Vns_GridPresentation_SpawnParams.OptionalOverrideRenderData`).

Unlike a scalar — where the `Params()` parameter keeps the source strong type and the ctor wraps it — a **container parameter is declared as the RETAINED type** (`Format_ParameterType`). AS has no `TArray<T>` → `TArray<TWeakObjectPtr<T>>` conversion and `TArray<TWeakObjectPtr<T>>(InX)` is not a valid wrapper call, so the alternative would be emitting element-wise loops into every generated ctor. The cost is that a caller holding a strong container converts at the call site; the ctor stays one assignment per field. Pinned by `…ParamsGenerator.PreservesWeakAndSoftObjectWrappers` (StrongSounds / OptionalStrongSound / StrongSoundsByName cases).

⚠ The `TArray` element form is proven in AS (in-tree usage predates this change). `TSet` / `TMap` / `TOptional` of a weak pointer are asserted at the SPELLING level by the unit test only — no in-tree AngelScript exercises those shapes yet, so the first entity script that declares one is also the first real compile test.
...
* **`StartupModule`** — broad `Delete_AllStubRecoveryFiles()` sweep before AS delegates are wired. Catches force-quit survivors that would otherwise collide with regenerated canonical output. **Retention rule (`Delete_StubRecoveryFiles_ForPatterns`):** a `_StubRecovery_*` file is only a STALE SURVIVOR once its canonical exists to supersede it. With gitignored canonicals (`*_EntitySpawnParams.as`) the stub IS the bootstrap for the upcoming AS compile — in commandlets this module loads BEFORE Hazelight's initial compile, so deleting a stub whose canonical is absent re-wedges the cook retry that synthesized it (pinned by the 2026-06-10 two-pass cook verification). The sweep therefore skips (and logs) any stub with no canonical alongside it; once the compile succeeds the canonical regenerates and the next sweep deletes the stub normally.
...
**The session boundary decides no-op vs defer.** The full-shape session set is PROCESS-scoped on purpose. A `NoMatchingSignatures` miss against a full shape written in THIS process means the signature was never compile-tested (same bulk drain) → no-op success, let the next compile decide; appending here would add junk overloads for calls the full shape already satisfies. The same error against an on-disk full-shape marker NOT in the session set (the headless cook-retry boundary) means that compile already ran against the full shape and still missed, so the caller genuinely diverges → return `FullShapeOnDiskStillMismatched` (Success = false, reason names the per-signature path) and let the dispatcher's error-text fallback append the per-signature overload ALONGSIDE the full shape. Returning success unconditionally swallowed that fallback and wedged the headless cook retry on `BB_CorridorGym.as`'s stale 25-arg `UBb_StoreDriver_EntityScript` call (2026-06-10). `Reset_SessionState_ForTests` models the process boundary; pinned by `…StubSynthesizer.Inject_SourceDerived_EndToEnd`.

**AssetRegistry stub:** Same shape — sibling `_StubRecovery_<MatchedAssetsFile>.as` containing the same `namespace assets { ... }` block. AS namespace-merge handles the rest. The choice of which `*Assets.as` file to land alongside is made by `Pick_BestSite_ByAssetPath` — prefix-match of the asset's package path against each candidate's `DiscoveryRoot`, longest-match wins. Falls back to first candidate when no root owns the asset on disk. **Deferred reshuffle:** even a successful Tier 1/2 synthesis can land the stub in the WRONG `*Assets.as` — the file scan picks the first namespace match before the asset-on-disk lookup identifies the owning DiscoveryRoot. `Did_SynthesizeAssetRegistryStub_ThisSession` exists so the Module fires a deferred `GenerateAllAssetRegistries` to reshuffle it.
...
**Force-quit between recovery and PostCompile:** Sibling files survive to next launch. AS compile succeeds again (the sibling is still on disk, merge-via-namespace still works, the JSON merge still works); `StartupModule`'s broad sweep then deletes any leftover stubs. This retention rule is also what makes the Rev 11 canonical quarantine force-quit-safe: a quarantined (deleted) canonical plus its rebuilt sibling boots exactly like a fresh clone. It is also *why* quarantine DELETES the canonical (after the forensic copy) rather than editing it — the canonical is rewritten from live reflection on every successful compile, and the retention rule keeps the rebuilt sibling alive across a force-quit. Fallback plan if hot-reload ever proves blind to a mid-session deletion: switch to an atomic truncate-in-place (one-line header) — mtime-detected, compiles to empty, PostCompile regen restores it. Known harmless edge: a plugin whose entity-script classes were ALL deleted gets no bucket from `Bucket_ClassesByPlugin`, so its quarantined canonical never regenerates and an empty sibling is retained each boot — log-visible, no functional impact. The `_StubRecovery_*` files double as the on-disk signal for the deferred-regen paths (`Maybe_RegenAssetRegistry_OnPostInit`, `Maybe_RegenDynamicHandleJson_OnPostInit`) to know they should fire a proper regen.
...
5. **Don't add an `ECk_AsParsedError_Kind` without also adding its `DeduplicateRoots` case.** The switch has no `default` on purpose — a new kind should force you to think about its key. MSVC does not warn (C4061/C4062 are off by default even at /W4), so a missing case compiles clean; the key is pre-seeded to a per-site `kind<N>|<file>:<line>:<col>` fallback so that slip degrades to over-attempting (which the convergence cap absorbs) rather than collapsing every root onto `""` and silently dropping all but the first. Key on whatever makes two roots genuinely distinct for the recovery they feed — for canonical-scoped recoveries that includes `FilePath`.
