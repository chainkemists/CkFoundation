...
| G12 | `FCkAsDhPreSeed::PreSeed_MissingDynamicHandles()` (StartupModule funnel) | boot-time DynamicHandle sibling pre-seed write (see *Boot-time DH pre-seed* below); a secondary skips with one Warning and behavior degrades to exactly today's heal path |
...
#### Boot-time DH pre-seed (prevention layer, G12)

`SelfHeal/CkAngelscriptGenerator_DhPreSeed.{h,cpp}`. The most common self-heal cycle is not corruption — it is the *designed* landing path for a pull that adds a new dynamic handle: the machine-local canonical JSON lacks the entry, the FIRST AS compile fails, and the modal-path heal costs a wasted full compile + heal + settle (~6-8s measured in BB) plus the Hazelight error modal, for every teammate on every such boot. The pre-seed produces the heal's *outcome* before the first compile:

- **When:** `StartupModule`, inside the self-heal arming block — AFTER the `Delete_AllStubRecoveryFiles()` sweep (which would otherwise delete the seed) and AFTER `Reset_CyclesRun()` (which clears the `Did_SynthesizeJsonStub` flag the pre-seed sets to arm the OnPostEngineInit regen — seeding before the reset wipes the flag and the canonical never converges; caught by the 2026-07-16 end-to-end verification boot). Still strictly before the first AS compile AND before Hazelight's hot-reload checker thread exists (the sibling is a `.json`, which that thread doesn't watch anyway — no mtime-baseline interaction in either direction).
- **How:** textual scan of the project + enabled-plugin `Script/` trees (`FCkAsSourceScanner` enumeration — `Generated/` and `_StubRecovery_*` excluded; comment/string-aware via `Blank_CommentsAndStrings`) for `asset <Name> of UCkDynamic_HandleDefinition` blocks; extract `TypeName` (+ explicit `ShortName`, else `ExtractShortNameFromTypeName` — registry-load parity); diff against the canonical's TypeNames; append the missing ones to the sibling via `FCkAsRecoveryDispatcher::Append_DynamicHandleStubEntries` — the SAME writer the heal path uses, so the entry shape cannot drift; then `Mark_JsonStubSynthesized()` arms the usual deferred regen (PostCompile/OnPostEngineInit strict-validator upgrade + canonical rewrite + sibling cleanup — all pre-existing rails).
- **Gates:** same opt-out family as the heal path (`-NoCkAsRegen` / `_EnableAsBootstrapSelfHeal`), plus ownership gate G12 inside the funnel — a secondary skips with one Warning and degrades to exactly today's behavior.
- **Fallback guarantee:** the pre-seed is an optimization layer over the heal, never a replacement. Anything the scanner misses (odd formatting, unreadable file) fails the compile exactly as today and the dispatcher recovers it. Scanner failures log at Verbose, never ensure.
- **Cost visibility:** the funnel logs the scan duration (`[DhPreSeed] Boot pre-seed scanned N declaration(s) ...`). If that duration ever approaches the ~6-8s cycle it prevents, the pre-seed has lost its reason to exist.
- **Log lines:** action → `[DhPreSeed] Pre-seeded {N} DynamicHandle stub entry(ies) from AS source scan: [...] -> <sibling>`; no-op → single Verbose line, no writes, no flag. `_probe_merge_conflict`'s DH leg asserts the pre-seed line (cold-start DH drift no longer reaches the dispatcher); the mid-session probes are untouched (pre-seed is boot-only).
...
**Force-quit between recovery and PostCompile:** Sibling files survive to next launch. AS compile succeeds again (the sibling is still on disk, merge-via-namespace still works, the JSON merge still works); `StartupModule`'s broad sweep then deletes any leftover stubs. This retention rule is also what makes the Rev 11 canonical quarantine force-quit-safe: a quarantined (deleted) canonical plus its rebuilt sibling boots exactly like a fresh clone. Known harmless edge: a plugin whose entity-script classes were ALL deleted gets no bucket from `Bucket_ClassesByPlugin`, so its quarantined canonical never regenerates and an empty sibling is retained each boot — log-visible, no functional impact. The `_StubRecovery_*` files double as the on-disk signal for the deferred-regen paths (`Maybe_RegenAssetRegistry_OnPostInit`, `Maybe_RegenDynamicHandleJson_OnPostInit`) to know they should fire a proper regen.
