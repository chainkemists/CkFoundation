...
// Polls <ProjectSaved>/CkAssetExporter/Requests/*.json, runs each (op = export | list | quit) through the dispatch
// layer, writes <ProjectSaved>/CkAssetExporter/Results/<basename>.json, then deletes the request. Publishes a
