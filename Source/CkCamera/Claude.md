...
settled every input anchor and before the transform-local settle barrier and Finalize.
...
- `FFragment_Camera::_ViewInfo` — the render authority, pulled by `UCk_CameraComponent::GetCameraView`.
- the **view anchor** (`UCk_Utils_Camera_UE::Get_ViewAnchor`) — an attachable child transform whose pose is
  enqueued as an ordinary `Request_SetTransform` and drained by the transform-local settle barrier. Scene-node-attach
