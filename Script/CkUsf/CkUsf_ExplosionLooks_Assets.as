...
// batch already carries, checked value-by-value against the sheets' §4 delta
// tables before reuse:
//
...
// CkFoundation/Source/CkParticles/Cookbook/NS_ExplosionGround.md §4.1 and
// NS_Bomb_Explosion.md §4.1. A delta table states the FULL inherited pair, so
...
// standing family gaps recorded in each recipe's §13. The Color_Core tint on
...
// Bubble emitter writes that channel as 0 (NS_Bomb_Explosion.md §5.9). So the
...
namespace CkUsf
{
    ...
    // does not exist and the GradientTrapezoid bake is a transcription rather than a stand-in (recipe §7).
    //
